...
The weights are computed similarly to the forward filter using the backward proposal density if an equivelent version of we use an auxiliary particle filter. We should though maybe change the normal distribution term in~\eqref{eqn:FWWeights} to account for the prior. We can also use a bootstrap like filter with the importance density: 
...
TODO: write about what is and is not included in the BW filter
TODO: write about likelihood evaluation
