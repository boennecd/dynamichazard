...
We can use the EM-algorithm \citep{dempster77} to estimate $\mat{Q}$ and $\vec{a}_0$ elements of $\vec{\theta} = \Lbrace{\mat{Q}, \mat{Q}_0, \vec{a}_0}$. We do this by running algorithm~\ref{alg:ONsmoother} for with the current $\vec{\theta}$. Then we compute the so-called \emph{smoothed additive functional} or just summary statistics:
