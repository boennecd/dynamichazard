...
system.time(pf_Fear_avg <- PF_EM(
  ...
  control = PF_control(
    ...
    method = "AUX_normal_approx_w_cloud_mean",
    n_max = 300, smoother = "Fearnhead_O_N", averaging_start = 150L,
    nu = 8, covar_fac = 1.1, n_threads = n_threads)))
```
...
We will run stochastic gradient descent a as in @polyak92 which is described 
in @cappe05 [page 414]. 
...
comp_obj <- PF_get_score_n_hess(pf_start)
n_runs <- 200L
state <- matrix(
