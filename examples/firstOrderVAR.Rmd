                ...
                avg_start = 100L)
{
  ...
  for(i in 1:n_runs){
    ...
    while((n_halfs <- n_halfs + 1L) <= n_halfs_max){
      state_i <- 
        state[i + 1L, ]  <- state[i, ]  + lr * drop(K %*% sc[-(1:dfix)])
      obs_i   <- 
        obs  [i + 1L, ]  <- obs  [i, ]  + lr * sc[1:dfix]
      lr <- lr / 2
      
      if(i > avg_start){
        idx <- avg_start:i
        state_i <- colMeans(state[idx, , drop = FALSE])
        obs_i   <- colMeans(obs  [idx, , drop = FALSE])
...
# step sizes
lrs <- .005 * (1:n_runs)^(-3/5)

out <- sgd(n_runs = n_runs, ns = ns, lrs = lrs, avg_start = 200L)
lls <- out$lls
o <- out$o
```
...
n_runs <- 400L
# number of particles to use
ns <- ceiling(exp(seq(log(200), log(500), length.out = n_runs)))
# step sizes
lrs <- .004 * (1:n_runs)^(-2/3)
...
aprxs <- replicate(10, {
  ...
  # return approximate score and standard erors 
  list(
    score = comp_out$score, 
    ses = suppressWarnings(rbind(
      full = sqrt(diag(solve(comp_out$neg_info))),
      # assuming expected gradient is zero
      `w/o gradient` = 
        sqrt(diag(solve(comp_out$neg_info - tcrossprod(comp_out$score)))))))
}, simplify = FALSE)
```
