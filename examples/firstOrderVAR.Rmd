...
knowing the log time at risk, $\vec{o}_t$, covariates, $X_t$ and $Z_t$, and latent states, $\vec{\alpha}_t$. 
...
can be found on 
[the github site for the package](http://github.com/boennecd/dynamichazard/examples).
All functions which assignments are not shown can be found on the github site.
...
We then simulate and plot the latent variables
...
system.time(pf_Brier <- PF_EM(
  ...
  control = PF_control(
    ...
    n_max = 60, smoother = "Brier_O_N_square",
...
approximate log-likelihood from the particle filter at the EM iterations are
...
system.time(pf_Fear <- PF_EM(
  ...
  control = PF_control(
    ...
    method = "AUX_normal_approx_w_cloud_mean",
    n_max = 60, smoother = "Fearnhead_O_N",
    Q_tilde = diag(.3^2, 2), n_threads = n_threads)))
```
...
However, there are many options for speeding up the computation time as e.g, 
...
The log-likelihood was still tending sloping towards the end but not monotonically
...
pf_Fear_more <- PF_EM(
  ...
  control = PF_control(
    N_fw_n_bw = 1000, N_smooth = 10000, N_first = 5000, eps = .001,
    method = "AUX_normal_approx_w_cloud_mean",
...
print(tail(pf_Fear$log_likes, 1), digits = 7)
```
