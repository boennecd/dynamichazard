...
system.time(pf_fearn <- PF_EM(
  ...
  Q_0 = diag(3^2, 2), Q = diag(1, 2), Fmat = diag(1, 2), 
  ...
  control = PF_control(
    ...
    Q_tilde = diag(.33^2, 2), n_threads = n_threads)))
```
...
pf_fearn_other <- PF_EM(
  ...
  by = 1, type = "VAR", model = "logit", max_T = n_periods,
  control = PF_control(
    N_fw_n_bw = 1000, N_smooth = 2500, N_first = 5000, eps = .0001,
    method = "AUX_normal_approx_w_cloud_mean", est_a_0 = FALSE,
    n_max = 250, smoother = "Fearnhead_O_N", 
...
```{r show_est_other}
chol(pf_fearn_other$Q)
pf_fearn_other$F
```

Clearly, something has gone. The log-likelihood at each EM-iterations are also 
off 
...
This is discouraging and I am not sure why the results are as they are.
