...
where correct or incorrect binning intervals refers to whether or not we bin at the same time where the coeffecient are simulated to change. For example, we bin correctly where we simulate the coeffecients to change at time $1,2,\dots,d$ and we estimate the coeffecient at time $1,2,\dots,d$. The models will be compared in terms of Brier score, median absolute residuals and standard deviation of the absolute residuals. All metrics will be reported on out-sample data or out-of-time data. All plots will have true coefficients as continuous lines while dashed lines are estimates
