...
The black line is the intercept while the colered lines are the coefficients for the covariates. Next, we can look at the number of failures in each simulation:
```{r}
...
Now, what happens if we get the binning wrong? This is the next experiment we will perform. Specifically, we will set the binning length to `0.1` instead `1` when we simulate. Thus, coefficients are updated at time $0, 0.1, 0.2, \dots$ and whether and individual dies is evulated at the same times when we simulate. However, the model will still use bins of length $1$
