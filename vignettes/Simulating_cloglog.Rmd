...
First, we need simulate our data sets. The parameters we will pass to `test_sim_func_poisson` in the file `R/test_utils.R` are:
...
We end up simulating `r arg_list$n_series` indviduals. Co-variates for invidiauls are changed with increaments of a $\sim \text{exp}(1)$ random variables. The co-variates are simulated to be from uniform distribution on [`r arg_list$x_mean + c(-.5, .5) * arg_list$x_range`] The state space vector chance with increaments of 1. We have `r arg_list$n_vars` parameters all starting at `r arg_list$beta_start`. Furter, we have an intercept starting at `r arg_list$intercept_start`. The standard deviation of the intercept and parameters are respectivly `r arg_list$sds`. The function we use to simulate with is printed below:

```{r}
# We are currenlty in the vignettes folder
getwd()
source("../R/test_utils.R")
test_sim_func_poisson
```


## Results
We are now ready to simulate and make plots of the fitted values and actual parameters This is done below `r n_plots` times. The dashed line are estimates and the continous lines are actual values of the parameters. The thick lines are the intercept and the thin ones are the parameters for the co-variates
