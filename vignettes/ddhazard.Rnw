...
<< echo=FALSE>>=
tryCatch({
  current_sha <- git_key <- system("git rev-parse --short HEAD", intern = TRUE)
...
The \code{ddhazard} function estimates a dynamic binary regression model where the parameters are assumed to time-varying and follow a random walk.



\subsection{Why and when to use the ddhazard}
The \code{ddhazard} function is intended for situation where you have a dynamic binary regression model with time-varying coefficients. The advantage of the state spaces methods used here is that you can extrapolate to time periods beyond the data used in estimation. An example is forecasting firm failures given the firms present accounting data. The task is to use the present data to estimate a model and forecast the likelihood of default for the firms in the following year. Another use of this package is as an alternative to other methods of modelling time-varying coefficients in survival analysis.
...
3) It is fast due to the \code{C++} implementation which uses \code{Armadillo} library and supports multithreading.
4) The methods a fast compared to e.g., sequential Monte Carlo alternatives which are also
...
The vignette here is the primary vignette where the models and estimation methods are explained. The package also contains other supplementary vignettes. \emph{Simulation study with logit model} presents a simulation study where the methods in this package are compared to each other and to Generalized Additive models. \emph{Comparing methods for time-varying logistic models} applies the methods to a real world data set. Both vignettes illustrate how to use the estimation function \code{ddhazard} and other functions in this package. They only use the discrete time model. This vignette also describes the continuous time model. The \emph{Bootstrap illustration} vignette shows how to use the \code{ddhazard\_boot} function which is a wrapper for the \code{boot} from the \code{boot} library. \emph{ddhazard Diagnostics} illustrates how the \code{residuals} and \code{hatvalues} functions can be used to check the model fit.
...
This is in the typical start-stop time format. The column \code{id} shows which individual the row belongs to. \code{tstart} is point at which the row is valid from and \code{tstop} is when the row is valid to. \code{event} is one if the individual dies at \code{tstop} and \code{x1} and \code{x2} are two covariates. Thus, the individual with id \code{1} dies at time \Sexpr{simple_ex$tstop[2]} while id \code{2} survives all the periods we observe. The models we look at will allow for censoring to deal handle an individual like id \code{2}.
...
%
where $\mathbb{Z}$ are the natural number $1,2,\dots$. We will refer to this as the \emph{discrete risk set} as we later introduce a continuous version. For simplicity we assume that we have removed all observation that are strictly inside an interval. I.e. those where:
...
%
$\vec{\alpha}_t$ is the state vector with the corresponding \emph{state equation}. Again, we will fix $\psi_t=1$. However, the \code{ddhazard} function is implemented to handle any equidistant interval length. That is, $\psi_t= \psi$ for a pre-specified constant $\psi$. Further, we define the observational equations covariance matrix as $\mat{H}_t(\vec{\alpha}_t) = \varpCond{\vec{y}_t}{\vec{\alpha}_t}$. The mean $\vec{z}_t(\vec{\alpha}_t)$ and variance $\mat{H}_t(\vec{\alpha}_t)$ are state dependent with:
$$\begin{aligned}
...
the linear predictor in the second order random walk as

$$(\mat R^\top \vec \alpha_t)^\top\vec x_{ijt} = \vec \xi_t^\top\vec\alpha_t$$%
%
...
The likelihood of the model where $\vec{\alpha}_t$ are observed can be written as follows by application of the Markov property of the model:
\begin{align*}
	\proppCond{\vec{\alpha}_0, \dots, \vec{\alpha}_{d}}{\vec{y}_{t},\dots,\vec{y}_T} & =
	 ...
	 & = p(\vec{\alpha}_0)\prod_{t = 1}^d \proppCond{\vec{\alpha}_t}{\vec{\alpha}_{t-1}}
		\prod_{(i,j) \in R_t} \proppCond{y_{ijt}}{\vec{\alpha}_t} + \dots
\end{align*}
...
@
%
Above, we estimate the model with a time intervals of length \code{by = 1}. The model is the logistic model which we introduced later. For now, let us see what happens if we increase the interval length by changing the \code{by} argument:
...
An EM algorithm is used to estimate the initial state space vector $\vec{\alpha}_0$ and the co-variance matrix $\mat{Q}$. Optionally $\mat{Q}_0$ is also estimated if \code{control = list(est\_Q\_0 = T, ...)}. Though this is discourage as we are estimating more parameters than observations without penalization. Define
...
We check the relative norm of the change in the state vectors to check for convergence. You can select the threshold for convergence by setting the \code{eps} element of the list passed to the \code{control} argument of \code{ddhazard} (e.g., \code{list(eps = 0.001, ...)}).
...
While \cite{fahrmeir92} does not observe improvements with multiple iterations, we find improvements in terms of out-of-sample prediction (for example by setting $\epsilon_{\text{NR}} = 10^{-2}$ or lower) with a moderate or large amount of observations. See the vignette "Simulation study with logit model" for details.

The value of $\zeta_0$ and $\epsilon_{\text{NR}}$ are set by respectively setting the elements \code{LR} and \code{NR\_eps} of the list passed to the \code{control} argument of \code{ddhazard}. By default, \code{LR = 1} and \code{NR\_eps = NULL} which yields a learning rate of 1 and a single Fischer scoring step. These arguments can be altered by setting e.g. \code{control = list(LR = 0.75, NR\_eps = 0.00001)} for a learning rate of 0.75 and a threshold in the Fisher Scoring of $10^{-5}$.
...
\end{aligned}$$
where $\xi>0$ is a small number. The default can be changed by altering the \code{denom\_term} in the list passed to the \code{control} argument of \code{ddhazard}. The approach is similar to how the \code{glmnet} package handles close to boundary estimates \citep[see]{friedman10}.


\section{Unscented Kalman Filter}
The UKF selects state vectors called \emph{sigma point} with given \emph{sigma weigths} chosen to match the moments of  observation equation. Thus, we approximate the density rather than approximating the observational equation. The idea is similar to a Monte Carlo method for state space models but where the state vectors are chosen deterministically rather than randomly drawn.

The motivation to use the UKF in place of the EKF is that we avoid the linerization error in the EKF. \cite{julier97} introduce a UKF that approximate the first two moments and up to fourth moment in certain settings. \cite{julier04} further develop the UKF and extended to what is later called \emph{the Scaled Unscented Transformation}. We will cover the the Scaled Unscented Transformation with the parametrizion from \cite{wan00} and formulas from \cite{menegaz16}.
...
\end{aligned}$$%
%
That is, we use the analytical solution. This version is both exact given the previous estimates $\emNotee{\vec{a}}{t - 1}{t - 1}$ and $\emNotee{\mat{V}}{t - 1}{t - 1}$ and computationally less demanding. Then we select $2q + 1$ so-called \emph{sigma points} (where $q$ is the dimension of the state equation) denoted by $\hvec{a}_0, \hvec{a}_1, \dots, \hvec{a}_{2q + 1}$ according to:
...
Another idea could be to try the Square-root Unscented Kalman filter suggested in \cite{Van01}. The idea is to use a QR decompositions and Cholesky updates to get more stable method. While \cite{Van01} shows that this scales  equally well in the dimension of the state vector we show below that it does not scale well with the number of individuals at risk, $n_t$. The prediction step is as before. Next, let
$$\begin{aligned}
...
The default in this package is $\kappa = q (1 + \alpha^2 (0.1 -1)) / (\alpha^2 (1 - 0.1))$ and can be altered by setting the list element \code{kappa} in the list passed as the \code{control} argument to \code{ddhazard}. For example, \code{control = list(kappa = 1, ...)} yields $\kappa = 1$. The default makes $W_0^{[m]} = 0.1$ such that all weights are positive. This ensures that $\emNotee{\mat{V}}{t}{t-1}$ and $\ukfNotee{y}{y}{t}$ are positive semi-definite. This follows since both are sum of outer products with positive weights and as $\widehat{\mat{H}}$ is a diagonal matrix with positive entries.

$0<\alpha \leq 1$ controls the spread of the sigma points. Notice that $\lambda + q \rightarrow 0^+$, $W_0^{[c]},W_0^{[m]}\rightarrow -\infty$ and $W_j^{[c]}, W_j^{[m]} \rightarrow \infty$ ($j > 0$) as $\alpha \rightarrow 0^+$. Thus, the lower the value of $\alpha$, the lower the spread but the higher the absolute weights. It is generally suggested to choose $\alpha$ small. See \cite{gustafsson12, julier04}. However, initial simulation studies showed that $\alpha = 1$ yields the smallest mean square error of estimated coefficients. Thus, this is the default. The parameter can be altered through the \code{alpha} element of the list passed to the argument \code{control} of \code{ddhazard}.

Lastly, $\beta$ is a correction term to match the fourth-order term in the Taylor series expansion of the covariance of the observational equation. \cite{julier04} show in the appendix that the optimal value with a Gaussian state equation is $\beta = 2$. Though, initial simulation showed that $\beta = 0$ yielded the best results and is therefore the default. It can be altered through the \code{beta} element of list passed to the argument \code{control} of \code{ddhazard}.
...
\section{Sequential approximation of the posterior mode (SMA)}
...
$$\emNote{\mat{V}}{t}{t}{k} = \emNote{\mat{V}}{t}{t}{k-1} -
  ...
  = \emNote{\mat{V}}{t}{t}{k-1} - \frac{\emNote{\mat{V}}{t}{t}{k-1}\vec{x}_{ij} g \vec{x}_{ij}^\top\emNote{\mat{V}}{t}{t}{k-1}}{
    1 + g / d_1}$$%
%
This method is selected by setting \code{method = "SMA"} in the list passed to the \code{control} argument of \code{ddhazard}.
...
This method is selected by setting \code{method = "GMA"} in the list passed to the \code{control} argument of \code{ddhazard}. You can change $k_{\text{max}}$ and $\epsilon$ by respectively setting the elements \code{GMA\_max\_rep} and \code{GMA\_NR\_eps} to the \code{control} argument. The above is sensitive to the choice of $\mat{Q}_0$. An extreme example is if we have no events in the first interval and only an intercept. Then setting $\mat{Q}_0$ to a diagonal matrix with large entries (in this case $\mat{Q}_0$ is a scalar) implies almost no restrictions on the intercept. Thus, it will be optimal to select a value tending towards minus infinity.
...
\begin{enumerate}
  ...
  \item We have piecewise constant instantaneous hazards given by $\exp(\vec{x}_{ij}^\top\vec{\alpha}_t)$ given an individual's current co-covariate vector $\vec{x}_{ij}$ and state variable $\vec{\alpha}_t$ (assuming that individual $i$'s $j$'th covariate is within time interval $t$).
\end{enumerate}
...
%
  ...
  \Lparen{\min\{ t, t_{ij} \} - \max\{ t - 1, t_{i,j-1} \}} + dots
\end{aligned}$$%
%
