...
We could directly minimize equaion~\ref{eqn:modeExact}. This is equivalent to a L2 penalized for a Generalized Linear Models (GLM) since only models from the exponential family is used in this package. This can be done with the usual  iteratively reweighted ridge regression. Every iteration can be done in $\bigO{n_tq^2 + q^3}$. We will go through computations in the following paragraphs. Define:
...
%
	...
	\tvec{g}(\vec{\alpha}) =  \tilde h'(\vec{\alpha})
		& = \emNotee{\mat{V}}{t}{t-1}^{-1} \vec{\alpha} - \left. \sum_{(i,j) \in R_t} \frac{\partial\log\proppCond{y_{ijt}}{\vec{\alpha}'}}{\partial \vec{\alpha}'} \right|_{\vec{\alpha}' = \vec{\alpha}} \\
		& = \emNotee{\mat{V}}{t}{t-1}^{-1} \vec{\alpha} - \mat{X}_t^\top \underbrace{\left. \frac{\partial\log\proppCond{\vec{y}_t}{\vec{e}'}}{\partial \vec{e}'} \right|_{\vec{e}' = \mat{X}_t \vec{\alpha}}}_{p'} \\
%
	\tmat{G}(\vec{\alpha}) = \tilde h''(\vec{\alpha})
		...
		& = \emNotee{\mat{V}}{t}{t-1}^{-1} - \mat{X}_t^\top \underbrace{\left. \frac{\partial\log\proppCond{\vec{y}_t}{\vec{e}'}}{\diag{\partial \vec{e}'\partial \Lparen{\vec{e}'}^\top}}\right|_{\vec{e}' = \mat{X}_t \vec{\alpha}}}_{p''}\mat{X}_t
\end{aligned}\end{aligned}\end{equation}

where $\diag{\cdot}$ returns a diagonal matrix with only the entries of the argument matrix in the diagonal. Algorithm~\ref{alg:GlobalMA} shows the iteratively re-weighted ridge regression. We make the following assumption to compare the algorithm~ with the EKF in algorithm~\ref{alg:EKFextra}. We restrict us model from  the exponential family with a density function of the form:

\begin{algorithm}
\caption{Correction step with global mode approximation with iteratively reweighted ridge regression.}\label{alg:GlobalMA}
\begin{algorithmic}[1]\raggedright
\Statex Set $\vec{a}^{(0)} =\emNotee{\vec{a}}{t}{t-1}$ and $k= 1$. Let $\vec{g}$ denote the inverse link function
\Repeat
\State %
$\begin{aligned}
\vec{a}^{(k)} &= \vec{a}^{(k - 1)} + \Lparen{\tmat{G}(\vec{a}^{(k - 1)})}^{-1}\Lparen{-\tmat{g}(\vec{a}^{(k - 1)})} \\
	&= \Lparen{\tmat{G}(\vec{a}^{(k - 1)})}^{-1}\Lparen{
		- \mat{X}_t^\top  p'' \mat{X}_t  \vec{a}^{(k - 1)}
		+ \mat{X}_t^\top p'} \\
	&= \Lparen{\emNotee{\mat{V}}{t}{t-1}^{-1} + \mat{X}_t^\top (-p'')\mat{X}_t}^{-1}\mat{X}_t^\top
		\Lparen{-p''}\Lparen{
		 \mat{X}_t  \vec{a}^{(k - 1)} -
		\Lparen{p''}^{-1}p'}
\end{aligned}$
...
\begin{equation}
	\tilde p'(d) = y_{ijt} - b'(d) = y_{ijt} - h(c) \qquad \tilde p''(d) = -b''(d) = - \frac{\varpCond{y_{ijt}}{d = \vec{x}_{ij}^\top\vec{\alpha}}}{\phi}
\end{equation}
