...
Next, we fit a model with an exponential hazard function with time-varying coefficients with the \code{ddhazard} function and plot the predicted coefficients. We will later cover what the input to \code{ddhazard} are and details on the estimation. The fit and plot is made as this point to illustrate the \code{ddhazard} function. The plot is shown in figure~\ref{fig:aids_plot_first}.
...
<<>>=
fit_M_step <- ddhazard(
  Surv(stop, event) ~ - 1 +          # No intercept
...
Notice that the difference is in the coefficient for the factor for \code{prevOI} and there is also some difference in the intercept in figure~\ref{fig:second_order_aids}. The cause may be that the there are very few events in the \code{noAIDS} group towards the end as the next code shows:

<<>>=
aids$stop_cut <- cut(aids$stop, breaks = c(0, 5, 10, 15, Inf))
ftable(xtabs(~ prevOI + I(event * (stop <= 19))
             + stop_cut, aids), row.vars = 1:2)
@
