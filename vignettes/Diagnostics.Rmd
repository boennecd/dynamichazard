...
This vignette will show examples of how the `residuals` and `hatvalues` functions can be used for an object returned by `ddhazard`. See the vignette `ddhazard` For details of the computations. First, we will present all the data sets we will use and the diagnostics which turned out to be potentially useful for the given data set. This is in the section called "First round of checks". Then we return to the data sets using the other methods. The latter part is included to illustrate how the function in this package work. This is in the section called "Second round of checks".

# First round of checks
## Data set 1: Prisoner recidivism

The first data set we will look at is an experimental study of recidivism of 432 male prisoners a year after being released from prison. The details of the data set is in [@rossi80]. The study randomly gave financial aid to the prisoners when they where released. The variables we will look at are:
...
The returned object is list with a matrix for each interval. Each matrix have a column for the hat values, the row number in the original data set and the id of the individual the hat values belongs to: 
...
We accumulate the hat values for each individual and plot them against time. The motivation to do this is to spot if a given individual has a lot high sum hat value. This could indicate that he has a large influence on the estimates
...
Some of the observations seems to be quite influential. We find the five individuals with the most extreme maximum value and plot the last record for each of these:

```{r}
max_cum <- tapply(hats_stacked$hats_cum, hats_stacked$id, max)
is_max <- names(max_cum)[order(max_cum, decreasing = T)[1:5]]
is_max # The id of the persons
...
We may further want to compare the likelihood of the two models to make sure that the new model has a greater or equal likelihood. However, the final model seems to converge towards a degenerated distribution if we keep iterating in the EM algorithm. Thus, the EM slowly converge towards the end making it infeasible to get a good estimate of the likelihood.  
...
ctrl = list(
    ...
    method = "SMA",             # Use rank-one approximation of posterior mode. 
...
The intercept drops in the first period which possibly is due to the initial high number of deaths right after the first heart attack. We further simply the model the model by removing `gender` variable which seems to be zero:
...
The list of matrices is un-handy so we have defined a function called `stack_residuals` stack the matrices, add the interval number and the id of that the observations belongs to in. The definition of the function is given at the end of this vignette.
...
We start by getting a reference to the `test_sim_func_logit` function in the `dynamichazard` package which is not exported. We will use it to simulate the each individuals series>
...
The errors seems more variable than before>
...
We will look at the Pearson residuals but at an individual level in the following paragraphs. We use the Rossi data set where we find the residuals as follows:
...
We may want to take a closer look at the individuals with a high maximum accumulated residual:
...
We can repeat the same procedure for the lowest accumulated residual as follows:
...
The red curves are the predicted coefficients without the previous discussed cases. Similar conclusion applies for doing the same with the Worcester Heart Attack Study data set
...
The first one has a high heart rate while the latter two has low BMI (below 18.5 is underweight). Another idea is to look at the average up to the given time of the hat values. The motivation is the two lines that end around the 5'th interval either because they die or are right censored. Hence, we normalize by the interval number and plot against time
