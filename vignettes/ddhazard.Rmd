...
3. We have piecewise constant instantaneous hazards given by $\exp(\vec{x}^T\vec{\alpha})$ given an individiauls current co-covariate vector $\vec{x}$ and state variable $\vec{\alpha}$
...
The methods with the binary outcome and the right clipped time variable with a jump term seem to do best on simulated data
...
These time variables are connected the stop time $T_i$ as follows. Suppose that all the interval has length one such that $t_{i,s} - t_{i,s-1} = 1$ if we do not have an event ($T_i > t_{i,s}$). Then: 
$$\begin{aligned}
...
We call these variables right clipped observation times because each $\Delta_{ij}$ is a right clipped exponential variable conditional that individual $i$ has survived up to time $t_{i,j-1}$. A point has to be made about how we compute the mean and the variance in the case of right censoring and in the case of death in the previous equations. Right censoring is treated by having $t_{i,l_i} < \Lceil{t_{i,l_i}}$. We only know that the individual survived up to time $t_{i,l_i}$ and do not know if he survived the entire time interval (which is $\Lceil{t_{i,l_i}}$) 
...
We will make a the following example to show the state vector will tend towards values such that the linear predictors goes to minuns infinity. Suppose that we use the UKF and we only have an intercept such that $\vec{x}_{is} = \vec{x} = (1)$ for all $i$ and $s$. Further, we use a first order random walk such that $\vec{\alpha}_j=(d_j)$. This implies that all the linear predictors are given by $\vec{x}^T\vec{\alpha}_j=d_j$. The predicted mean for given observation can be $t_{i,s} - t_{i,s - 1}$ at most since we do not round up. Further, the predicted outcome will tend towards $t_{i,s} - t_{i,s - 1}$ as the linear predictor, $d_j$, tend towards minus infinity. 
...
where we can show that $\ukfNotee{x}{y}{t}\ukfNotee{y}{y}{t}^{-1}$ (which is a scalar) is negative in the example given here. Hence, $d_1,d_2,\dots$ will decrease in every iterations of the UKF. On the other hand, if we do round up in the case of death then the residuals, $\vec{y}_j - \overline{\vec{y}}$, can be negative in the case of death. Consequently, a death can increase $d_j$ value if the residual is negative. Another way to reason about this is that the individual could have survived the entire time interval, $\Lceil{t_{i,s}}$, but only survived, $t_{i,s}$. Thus, we use $\bar t_{i,j}$
...
There are two points to be made here. Firstly, we do not round up by using $\bar t_{i,j}$ in place of $t_{i,j}$ when we have a death. Thus, we do take into account the moment the person dies at in that $\log h^Y_{i,j}(\vec{x}_{i,j}^T \vec{\alpha}_s)$ is the likelihood of dying sometime in the period $t_{i,j} - t_{i,s-1}$. Moreover, an individual can have multiple observations in the same time interval if he does not die but his co-variate vectors change within the bin. Thus, we do not have issue with going from event times to binary outcomes as with the logistic model 
