...
# Intro
This note will cover the `ddhazard` function in `dynamichazard` library. You can install the version of the library used to make this vignettes from github with the `devtools` library as follows:
...
where $\delta$ is small like $10^{-9}$. The defaults are $l = 1$ and $\epsilon_{\text{NR}} = \infty$. The latter yield only a single iteration in the scorring step. Selecting $l < 1$ in case of divergence can resovle the issue. Thus, the `ddhazard` function will try learning rates of $l, l\zeta, l\zeta^2 \dots l\zeta^w$ for given $0<\zeta<1$. The process is stopped when we succeed to fit the model or $w$ is reach without succeeding in estimating the model. Further, while [@fahrmeir92] does not observe improvements with multiple repetitions, we find improvements in terms of mean square error of the state vector by taking multiple steps (for example by setting $\epsilon_{\text{NR}} = 10^{-2}$ or lower) (TODO: Insert reference to simulation note)
...
## Recommandations
In general, choosing $\mat{Q}_t = k\mat{I}$ with a large value $k$ and $\mat{Q} = d\mat{I}$ for $d$ small seems to work for a lot of data set. The exact value of $k$ and $d$ seems not to be too important. For instance, one dataset yielded almost the same result in terms of mean square error when $k \in [1, 10^3]$ and $d \in [10^{-6},10^{-1}]$


# Uncented Kalman Filter
The UKF selects state vectors called *sigma point* with given *sigma weigths* such that to match the moments of  observational equation. Thus, we approximate the density rather than approximating the observational equation
...
2) The author shows that the above formulation is what the author coins as consistent. That is, it yields the same estimate as a regular Kalman filter when you use the mehtod on a linear Gaussian state space model
3) They provide a simulation study for a non-linear system where the above method yields the best result in terms of average square error for the state space vectors
...
As pointed out in [@julier04] and [@menegaz16], the regular Kalman filter prediction formulas can be used when the state equation is a linear Gaussian model. Thus, we can simplify the filter step to: 
$$\begin{aligned}
  \emNotee{\vec{a}}{t}{t - 1} = \mat{F} \emNotee{\vec{a}}{t - 1}{t - 1}, \quad \\
  \emNotee{\mat{V}}{t}{t - 1} = \mat{F}\emNotee{\mat{V}}{t - 1}{t - 1}\mat{F}^T + \mat{R}\mat{Q}\mat{R}^T
\end{aligned}$$
...
# Fixed effects
This section will cover how the fixed effects (non time varying) are estimated. The likelihood we are working with can be written as follows by application of the markovian property of the model:
$$\begin{split}
...
This is what we approximately find the expectation of in the E-step of the EM-algorithm where $\mat{Q}$, $\mat{Q}_0$ and $\vec{\alpha}_0$ are assumed fixed. Denote the entries of the latter vector and two matrices by $\vec{\theta}$. Then the M-step proceeds by maximizing $\vec{\theta}$ conditional on the estimates in the E-step. Expanding the log likelihood yields:  
...
## Fixed effects
Fixed effects in the M-step is estimated like a regular GLM with a logistic link function where each individuals' observation will get an observation in the new model with an individual offset for each bin the observation is in. Say for instance that we want to fit: 
```{r, echo=FALSE}
set.seed(1010101012)
data_frame <- 
...
Let $T_i$ denote the random death time of the $i$'th individual. Lastly, let $y_{ij} = 1_{\{T_i \in (t_{i,j - 1}, t_{i,j}]\}}$ be the indicator for whether the $i$'th individual dies in period $(t_{i,j - 1}, t_{i,j}]$
...
We could implement the above. I have done that for the EKF. There is and issue though with divergence. The cause it that the entries of $\mat{K}$ can become numerically large. In particular, we can show that: 
...
We see that certain combinations of $(\delta, \eta)$ cause the entries of $\mat{K}$ to be large. We will illustrate why this may be an issue in the following paragraphs. Notice that the deriatives of the two link functions (the entries under `Deriv ...`) have opposing signs while all the entries of $\mat{K}$ (column K11, K12, K22) are positive. Thus, there is a canceling effect in the computation of $\mat{U}_t(\vec{\alpha}_t)$. Hence, $\emNotee{\mat{V}}{t}{t}$ seems not to be an issue in the sense that it "stays around" sensible values before divergence

The update from $\emNotee{\vec{a}}{t}{t-1}$ to $\emNotee{\vec{a}}{t}{t}$ seems to be the cause divergence. Re-call that:
...
The factor for the residual time is the product of `r grid[ex, "Deriv link Delta"]`, (`r grid[ex, "K22"]` plus `r grid[ex, "K12"]`) and (1 minus `r grid[ex, "Link Delta"]`). The result is `r grid[ex, "Deriv link Delta"] * (grid[ex, "K22"] + grid[ex, "K12"]) * (1 - grid[ex, "Link Delta"])`. Meanwhile the factor for the information matrix $\mat{U}_t(\vec{\alpha})_t$ is `r grid[ex, "Deriv link Y"]^2 * grid[ex, "K11"] + 2 * grid[ex, "Deriv link Y"] * grid[ex, "Deriv link Delta"] * grid[ex, "K12"] + grid[ex, "Deriv link Delta"]^2 * grid[ex, "K22"]` ($\Lparen{\Lparen{h^y_i}'(\eta_i)}^2\MyInd{\mat{K}_{11}}{ii} + 2\cdot\Lparen{h^y_i}'(\eta_i)\cdot\Lparen{h^\Delta_i}'(\eta_i)\MyInd{\mat{K}_{12}}{ii} + \Lparen{\Lparen{h^\Delta_i}'(\eta_i)}^2\MyInd{\mat{K}_{22}}{ii}$). Recall that:
...
Two other solutions are: 1) decrease the learning rate until we get a fit or 2) notice the entries of the inverse covariance matrix, $\mat{K}$, are large due the division by $1-\rho_i^2$. Hence, scale $\mat{K}$ by $1-\rho^2_i$ to get:
...
The inviduals covaraites are simulated from a uniform distribution on $[0,1]$. The link function is the exponential model suggested above. Each bin has length 1 and we stop observering at time `r arg_list$t_max`
...
As a final example, here is the output from the E-step of the EKF for the head neck data in [@fahrmeir94]. `U` is the diagonal of the information matrix $\mat{U}$:
...
[OMITTED OUTPUT FOR NEXT ITERATIONS]
...
## Diagnostics
I am thinking of making a another vignettes with diagnostics. It will contain raw residuals, Pearson residuals, non-standardized and standardized state space error. The motivation to keep it in a separate vignette is to include more examples and keep it as more of an example of how to use the package
