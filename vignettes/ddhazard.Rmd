...
A draw back to this model is that it will not work if the reported time scale is coarse as we cannot distinguish between a change of bin, new covariates vector, right censoring or death when the times coincides. As an extreme example, we cannot use this method if all times are reported on the grid of integers $1,2,\dots$ and we use bins of length $1$. You can estimate with the right truncated time method by setting the argument `model = "exponential_trunc_time_only"` in the call to `ddhazard`
...
You can estimate with the binary outcome method by setting the argument `model = "exponential_binary_only"` in the call to `ddhazard`
...
You can estimate with the right truncated time variable by setting the argument `model = "exponential_trunc_time_w_jump_only"` in the call to `ddhazard`
...
You can estimate with the tuples method by setting the argument `model = "exponential_combined"` in the call to `ddhazard`
