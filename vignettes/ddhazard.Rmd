...
Let $\vec{x}_{it}$ denote the co-variate vector for indvidual $i$ at time $t$ and let $Y_{it}$ be the random variable for whether the $i$'th individual dies at time $t$. For given parameters at time $t$ denoted by $\vec{\alpha}_t$ the probablity of death is:
...
$\vec{y}_t$ is the binary outcome and the associated equation is the *observational equation*. $\sim (a,b)$ denotes a random variable(s) with mean (vector) $a$ and variance (co-variance matrix) b. $\vec{\alpha}$ is the state vector with the coresponding *state equation*
...
The state equation is implemented with a 1. and 2. order random walk. For the first order random walk $\mat{F} = \mat{R} = \mat{I}_m$ where $m$ is the dimension of the state vector. As for the second order random walk, we have:
$$\mat{F} = \begin{pmatrix} 
  2\mat{I}_m & - \mat{I}_m \\ \mat{0}_m & \mat{I}
\end{pmatrix},  \qquad 
\mat{R} = \begin{pmatrix} \mat{I}_m \\ \mat{0}_m \end{pmatrix}$$

where $\mat{0}_m$ is a $m\times m$ matrix with zeros in all entries. The models are estimated by an Extended Kalman filter (EKF) and an Uncented Kalman filter (UKF). The method is chosen by passing list `control` parameter with an `list(method = "EKF", ...)` or `list(method = "UKF", ...)` respectivly
...
\emNotee{\mat{V}}{t}{k} = \expecpCond{\mat{V}_t}{\mat{Y}_k},  \mat{Y}_k = (\vec{y}_{1k}, \dots, \vec{y}_{sk}) $$
...
where $T$ is the last period we observe and supercripts $\cdot^{(k)}$ is used to destinguish the estimates from each iteration of the EM-algorithm. The required input to start the E-step is initial mean vector $\widehat{\vec{a}}_0^{(k-1)}$ and co-variance matrices $\widehat{\mat{Q}}_0^{(k - 1)}$ and $\widehat{\mat{Q}}_0^{(k - 1)}$. Given these input, we compute the folowing estimates either using the EKF or UKF:
...
## M-step
The M-step is used to compute updated mean $\widehat{\vec{a}}_0^{(k)}$ and co-variance matrices $\widehat{\mat{Q}}_0^{(k - 1)}$ and $\widehat{\mat{Q}}_0^{(k - 1)}$ (the latter being optional). These are computed by:
$$\begin{aligned}
\widehat{\vec{\alpha}}_0^{(k)} &= \emNote{\vec{a}}{0}{d}{k}\\
\widehat{\mat{Q}}_0^{(k)} &= \emNote{\mat{V}}{0}{d}{k} \\
    \widehat{\mat{Q}}^{(k)} &= \frac{1}{d}	\sum_{t = 1}^d \left( 
      \left( \emNote{\vec{a}}{t}{d}{k} - \mat{F}\emNote{\vec{a}}{t - 1}{d}{k} \right)
      \left( \emNote{\vec{a}}{t}{d}{k} - \mat{F}\emNote{\vec{a}}{t - 1}{d}{k} \right)^T \right. \\
    &\hspace{43pt} + \emNote{\mat{V}}{t}{d}{k} - \mat{F}\mat{B}_t^{(k)}\emNote{\mat{V}}{t}{d}{k} - 
      \left( \mat{F}\mat{B}_t^{(k)}\emNote{\mat{V}}{t}{d}{k} \right) ^T + 
      \mat{F}\emNote{\mat{V}}{t - 1}{d}{k}\mat{F}^T
      \left. \vphantom{\left( \emNote{\vec{a}}{t}{d}{k} - \mat{F}\emNote{\vec{a}}{t - 1}{d}{k} \right)^T} \right)
\end{aligned}$$
...
Going to the detail, we start with EKF with filter step:
$$\begin{aligned}
  \emNotee{\vec{a}}{t}{t - 1} = \mat{F} \emNotee{\vec{a}}{t - 1}{t - 1}, \quad \\
  \emNotee{\mat{V}}{t}{t - 1} = \mat{F}\emNotee{\mat{V}}{t - 1}{t - 1}\mat{F}^T + \mat{Q}
\end{aligned}$$
...
## Divergence
Intial testing showed that the EKF had issues divergence for some data set. The cause of divergence is overstepping in the correction step where we update $\emNotee{\vec{a}}{t}{t}$. That is, the signs of the margins of $\emNotee{\vec{a}}{t}{t}$ tended to alter between $t-1, t, t+1$ etc. and the absolute values tended to increase.  The following section describes a solution to this issue
...
$$\ukfNotee{a}{b}{t}= \expecpCond{(\vec{a}_t - \overline{\vec{a}}_t)(\vec{b}_t - \overline{\vec{b}}_t)^T}{\mat{Y}_k}$$
...
$$\begin{aligned}
  \wvec{a}_0 &= \emNotee{\vec{a}}{t-1}{t-1} \\
  \wvec{a}_{i} &= \emNotee{\vec{a}}{t-1}{t-1} + \sqrt{m + \lambda} \left(\sqrt{\emNotee{\mat{V}}{t - 1}{t - 1}}\right)_i \\
  \wvec{a}_{i + m} &= \emNotee{\vec{a}}{t-1}{t - 1} - \sqrt{m + \lambda} \left(\sqrt{\emNotee{\mat{V}}{t - 1}{t - 1}}\right)_i
...
$$\begin{aligned}
  ...
  \emNotee{\mat{V}}{t}{t-1} &= \mat{Q} + \sum_{i=0}^{2m} W_i^{(c)}
    (\wvec{a}_i - \emNotee{\vec{a}}{t}{t-1})(\wvec{a}_i - \emNotee{\vec{a}}{t}{t-1})^T \\
  & = \mat{Q} + \Delta\wmat{A}\diag{\vec{W}^{(c)}}\Delta\wmat{A}^T
\end{aligned}$$
...
We still need to select the hyperparameters $\kappa$, $\alpha$ and $\beta$. We will cover these in the given order. $\kappa$ is usually set to $\kappa = 0$ or $\kappa = 3 - m$ which [@julier97] state is a "* useful heuristic*" when the state equation is Gaussian. The default is 0 and can be alter by setting the list element `kappa` passed as the `control` argument to `ddhazard`. For example, `control = list(kappa = 1, ...)` yields $\kappa = 1$.
...
### Binning
Insert graph and make points. Motivates the exponential model
