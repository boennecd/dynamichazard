...
where $\mat{0}_m$ is a $m\times m$ matrix with zeros in all entries. The vector in the state equation is ordered as $\vec{\alpha}_t = (\wtvec{\alpha}_t^\top, \wtvec{\alpha}_{t-1}^\top)^\top$ to match the definition of $\mat{F}$ and $\mat{R}$ where the tilde is added to indicate the coefficients used when computing the linear predictor. The likelihood of the model where $\vec{\alpha}_t$ are observed can be written as follows by application of the markovian property of the model:
$$\begin{split}
...
$$\begin{aligned}
	...
	&  - \frac{1}{2} \sum_{t = 1}^d \Lparen{\vec{\alpha}_t - \mat{F}\vec{\alpha}_{t - 1}}^\top\psi_t^{-1}\mat{Q}^{-1} \Lparen{\vec{\alpha}_t - \mat{F}\vec{\alpha}_{t - 1}} \\ 
...
$$\begin{aligned}
  ...
  \Delta\wmat{Y} = \wmat{Y} - \overline{\vec{y}} \vec{1}^\top, \qquad 
  \wmat{H} = \sum_{j=0}^q W_j^{[c]}\mat{H}_t(\wmat{a}_j) \\
%
  \Delta\wmat{A} &= (\wvec{a}_0, \dots, \wvec{a}_{2q}) - \emNotee{\vec{a}}{t}{t-1}\vec{1}^\top \\
%
  \ukfNotee{y}{y}{t} &= \sum_{j=0}^{2q} W_j^{[c]} \left(
    (\wvec{y}_j - \overline{\vec{y}})(\wvec{y}_j - \overline{\vec{y}})^\top + \wmat{H}\right)
  = \Delta\wmat{Y}\diag{\vec{W}^{[c]}}\Delta\wmat{Y}^\top + \wmat{H} \\
  \ukfNotee{\alpha}{y}{t} &= \sum_{j=0}^{2q} W_j^{[cc]} 
    (\wvec{a}_j - \emNotee{\vec{a}}{t}{t-1})(\wvec{y}_j - \overline{\vec{y}})^\top
  = \Delta\wmat{A}\diag{\vec{W}^{[cc]}}\Delta\wmat{Y}^\top
\end{aligned}$$
...
$$\begin{aligned}
  \tilde{\vec{y}} &= \Delta \wmat{Y}^\top \widehat{\mat{H}}^{-1}(\vec{y}_t - \overline{\vec{y}}) \\ 
    \mat{G} &= \Delta\wmat{Y}^\top\widehat{\mat{H}}^{-1}\Delta\wmat{Y} \\
  ...
  \emNotee{\mat{V}}{t}{t} &= \emNotee{\mat{V}}{t}{t - 1} - 
    \Delta\wmat{A}\diag{\vec{W}^{(cc)}}\mat{L}\diag{\vec{W}^{(cc)}}\Delta\wmat{A}^\top
\end{aligned}$$
...
$$\begin{aligned}
  &\mat{C} = \qr{\begin{bmatrix}
    \sqrt{W_1^{[c]}}\Lparen{\wvec{y}_1 - \overline{\vec{y}}} &
    \sqrt{W_2^{[c]}}\Lparen{\wvec{y}_2 - \overline{\vec{y}}} & \dots &
    \sqrt{W_{2q}^{[c]}}\Lparen{\wvec{y}_{2q} - \overline{\vec{y}}} & \sqrt{\widehat{\mat{H}}}
    \end{bmatrix}} \\
%
  &\mat{C} \leftarrow \cholup{\mat{C}, \wvec{y}_0 - \overline{\vec{y}}, \text{sign}\Lparen{W_0^{[c]}} \sqrt{\vert W_0^{[c]}\vert}} \\
% 
  &\ukfNotee{\alpha}{y}{t} = \Delta\wmat{A}\diag{\vec{W}^{[cc]}}\Delta\wmat{Y}^\top \\
% 
...
$$\begin{aligned}
  &\mat{C} = \qr{\begin{bmatrix}
      \sqrt{W_0^{[c]}}\Lparen{\wvec{y}_1 - \overline{\vec{y}}} &
      \sqrt{W_2^{[c]}}\Lparen{\wvec{y}_2 - \overline{\vec{y}}} & \dots &
      \sqrt{W_{2q}^{[c]}}\Lparen{\wvec{y}_{2q} - \overline{\vec{y}}} & \sqrt{\widehat{\mat{H}}}
    \end{bmatrix}} \\
%
  &\begin{aligned} \Rightarrow  \mat{C}^\top\mat{C} & = 
    ...
    \end{bmatrix}
    \begin{bmatrix}
      \sqrt{W_0^{[c]}}\Lparen{\wvec{y}_1 - \overline{\vec{y}}}^\top \\
      \sqrt{W_2^{[c]}}\Lparen{\wvec{y}_2 - \overline{\vec{y}}}^\top \\ \vdots \\
      \sqrt{W_{2q}^{[c]}}\Lparen{\wvec{y}_{2q} - \overline{\vec{y}}}^\top \\ 
      \sqrt{\widehat{\mat{H}}}
    \end{bmatrix} \\
    & = \Delta\wmat{Y}\diag{\vec{W}^{[c]}}\Delta\wmat{Y}^\top + \wmat{H} = \ukfNotee{y}{y}{t} 
  \end{aligned} \\
% 
  &\ukfNotee{\alpha}{y}{t} = \Delta\wmat{A}\diag{\vec{W}^{[cc]}}\Delta\wmat{Y}^\top \\
% 
...
$$\begin{aligned}
  &\mat{C} = \qr{\begin{bmatrix}
    \sqrt{W_0^{[c]}}\Lparen{\wvec{y}_1 - \overline{\vec{y}}} &
    \sqrt{W_2^{[c]}}\Lparen{\wvec{y}_2 - \overline{\vec{y}}} & \dots &
    \sqrt{W_{2q}^{[c]}}\Lparen{\wvec{y}_{2q} - \overline{\vec{y}}}
    \end{bmatrix}} \\
%
  &\mat{C} \leftarrow \cholup{\mat{C}, \widehat{\mat{H}}, 1}
\end{aligned}$$

In this case, we first find $\mat{C} \in \mathbb{R}^{(2q + 1)\times n_t}$ and then make a rank-$n_t$ update. The rest of the computations are in-expensive relative to the number of observations, $n_t$, since we reduce the dimension of $\mat{C} \in \mathbb{R}^{(2q + 1)\times n_t}$. In general, the key is that we want a Cholesky decomposition (or another decomposition) of $\Delta\wmat{Y}\diag{\vec{W}^{[c]}}\Delta\wmat{Y}^\top + \wmat{H}$ which is easy to compute and ease the rest of the computations and storage requirements
...
This method is selected by setting `method = post_approx` in the list passed to the `control` argument of `ddhazard`
...
Number 1 and 2 are true for the binomial model and true for all exponential families where the form though will change. That is, we have to change the likelihood expression of $-\log \proppCond{y_{ijt}}{\vec{\alpha}}$. Step 2 may further be at a limit as is the case for the binomial model since for $y_{ijt} = 1$ the limit is $0$ at $b \rightarrow \infty$ and for $y_{ijt} = 0$ the limit is $0$ for $b \rightarrow -\infty$. The convex polynomial will eventually dominate the slope of the logit function which tends towards zero. Further, the learning rate $\zeta_0$ the decrease factor $\zeta$ as in the EKF can be used here to by changing the correction step to:
...
$$\tilde{\vec{y}} = \Delta \wmat{Y}^\top \widehat{\mat{H}}^{-1}(\vec{y}_t - \overline{\vec{y}}) \qquad
    \mat{G} = \Delta\wmat{Y}^\top\widehat{\mat{H}}^{-1}\Delta\wmat{Y}$$
    
with

$$\tilde{\vec{y}} = \Delta \wmat{Y}^\top \mat{E}_t\widehat{\mat{H}}^{-1}(\vec{y}_t - \overline{\vec{y}}) \qquad
    \mat{G} = \Delta\wmat{Y}^\top\mat{E}_t\widehat{\mat{H}}^{-1}\Delta\wmat{Y}$$
...
Suppose now that we assume that some of the effects are fixed such that we replace the linear predictor $\vec{x}_{it}^\top\vec{\alpha}_t$ by $\wtvec{x}_{ij}^\top\widetilde{\vec{\alpha}}_t + \bvec{x}_{ij}^\top\vec{\gamma}$ where $\vec{\gamma}$ is the fixed coefficients and $\bvec{x}_{it}$ are the corresponding covariates. The new definition of $l_{ijt}$ is:

$$l_{ijt}(\widetilde{\vec{\alpha}}_t, \vec{\gamma}) = y_{ijt} \log h(\wtvec{x}_{ij}^\top \widetilde{\vec{\alpha}}_t + \bvec{x}_{ij}^\top\vec{\gamma}) + (1 - y_{ijt}) 
	\log \Lparen{1 - h(\wtvec{x}_{ij}^\top \widetilde{\vec{\alpha}}_t + \bvec{x}_{ij}^\top\vec{\gamma})}$$
...
3. $\wtvec{x}_{ij}^\top \wtvec{a}_t$ acts like offsets in the M-step when we estimate $\vec{\gamma}^{(k)}$
...
%
  l_{ijt}(\vec{\alpha}_t) =& (\vec{x}_{ij}^\top\vec{\alpha}_t)^{y_{ijt}} -\exp\Lparen{\vec{x}_{ij}^\top\vec{\alpha}_t}
...
$$\log\Lparen{\proppCond{t_i}{\vec{\alpha}_0,\dots,\vec{\alpha}_d}} = 
  ...
  -\int_0^{t_i}\exp\Lparen{\vec{x}_i(u)^\top\vec{\alpha}(u)}u\, du$$
...
$$\begin{aligned} 
h^{\tilde Y}_{i,j}(\vec{x}_{i,j}^\top \vec{\alpha}_{s}) =  1 -  \exp\Lparen{ - \exp(\vec{x}_{i,j}^\top \vec{\alpha}_{s}) 
...
$$\delta = \bar t_{i,j}-t_{i,j-1}, \qquad \lambda = \exp \Lparen{\vec{x}_{i,j}^\top \vec{\alpha}_s}, \qquad
...
$$\vec{y}_t = \Lparen{y_{1,i_{1,1}}, \Delta_{1,i_{1,1}},
  y_{1,i_{1,2}}, \Delta_{1,i_{1,2}} \dots y_{m,i_{m,k}}, \Delta_{m,i_{m,k}}}^\top$$
...
\mat{U}_s(\vec{\alpha}_s) = \dot{\vec{z}}_s(\alpha_s) \mat{H}_s(\vec{\alpha}_s)^{-1} \dot{\vec{z}}_s(\alpha_s)^\top
\end{array}$$
...
\vec{u}_s(\vec{\alpha}_s) = \sum_{l,j} 
  &\vec{x}_{l,j}\Lparen{h^{\tilde Y}_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\Lparen{\MyInd{\mat{K}_{l,j}}{1,1} + \MyInd{\mat{K}_{l,j}}{1,2}}(y_{l,j} - h^{\tilde Y}_{l,j}(\vec{x}_{l,j}^\top\vec{\alpha}_s)) \\
  &\hspace{4pt}+\vec{x}_{l,j}\Lparen{h^\Delta_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\Lparen{\MyInd{\mat{K}_{l,j}}{2,2} + \MyInd{\mat{K}_{l,j}}{1,2}}(\Delta_{l,j} - h^\Delta_{l,j}(\vec{x}_{l,j}^\top\vec{\alpha}_s))
\end{aligned}$$
...
\mat{U}_s(\vec{\alpha}_s) = \sum_{i=1}^r 
  &\vec{x}_{l,j}\Lparen{\Lparen{\Lparen{h^{\tilde Y}_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)}^2\MyInd{\mat{K}_{l,j}}{1,1} +
    \Lparen{h^{\tilde Y}_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\cdot\Lparen{h^\Delta_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\MyInd{\mat{K}_{l,j}}{1,2}}\vec{x}_{l,j}^\top\\
  & \hspace{4pt}+\vec{x}_{l,j}\Lparen{\Lparen{h^{\tilde Y}_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\cdot\Lparen{h^\Delta_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\MyInd{\mat{K}_{l,j}}{1,2} +
    \Lparen{\Lparen{h^\Delta_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)}^2\MyInd{\mat{K}_{l,j}}{2,2}}\vec{x}_{l,j}^\top
\end{aligned}$$
...
where $\lambda_{l,j} = \exp(\vec{x}_{l,j}^\top\vec{\alpha}_s)$. Though, it is numerically more stable to reduce the following three factors where the two first are used for the score vector and the last factor is used for the information matrix

$$\begin{aligned}
&\Lparen{h^{\tilde Y}_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\Lparen{\MyInd{\MyInd{\widetilde{\mat{K}}_{l,j}}{11}}{ii} + \MyInd{\widetilde{\mat{K}}_{l,j}}{1,2}} \\
&\Lparen{h^\Delta_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\Lparen{\MyInd{\widetilde{\mat{K}}_{l,j}}{2,2} + \MyInd{\widetilde{\mat{K}}_{l,j}}{1,2}}\\
&\Lparen{\Lparen{h^{\tilde Y}_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)}^2\MyInd{\MyInd{\widetilde{\mat{K}}_{l,j}}{11}}{ii} +
    2\Lparen{h^{\tilde Y}_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\cdot\Lparen{h^\Delta_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)\MyInd{\widetilde{\mat{K}}_{l,j}}{1,2} + 
    \Lparen{\Lparen{h^\Delta_{l,j}}'(\vec{x}_{l,j}^\top\vec{\alpha}_s)}^2\MyInd{\widetilde{\mat{K}}_{l,j}}{2,2}
\end{aligned}$$
