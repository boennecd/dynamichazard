...
### Kalman Filter
The standard Kalman filter is carried out by recursively doing two steps. This also applies for the EKF and UKF used in the E-step. Thus, this paragraph is included to introduce general notions. The first step in the Kalman Filter is the *filter step* where we estimate $\emNotee{\vec{a}}{t}{t - 1}$ and $\emNotee{\mat{V}}{t}{t - 1}$ based on $\emNotee{\vec{a}}{t - 1}{t - 1}$ and $\emNotee{\mat{V}}{t - 1}{t - 1}$. Secondly, we carny out the *correction step* where we estimate $\emNotee{\vec{a}}{t}{t}$ and $\emNotee{\mat{V}}{t}{t}$ based on $\emNotee{\vec{a}}{t}{t - 1}$ and $\emNotee{\mat{V}}{t}{t - 1}$ and the observations. We then repeat the process until $t=d$
...
The motivation to use the UKF in place of the EKF as we avoid the linerization error in the EKF. [@julier97] introduces a UKF that approximate the first two moments and up to fourth moment in certain settings. [@julier04] further develop the UKF and extend to what is later called *the Scaled Unscented Transformation*. We will cover the the Scaled Unscented Transformation with the paramatization from [@wan00] and formulas from [@menegaz16]

The reason why the UKF has received a lot of attention (especially in in engineering) is for settings where the observation equation is complicated since the UKF does not require that computation of the Jacobian matrix. However, deriving the Jacobian matrix for the models in this package is not difficult
...
$\mat{Q}$ has a similar effect although it is harder to illustrate with a small example as it occurs in an intermediate computations in the UKF. Question is then how to select $\mat{Q}$ and $\mat{Q}_0$. Based on experince, it seems that $\mat{Q}_0$ should be a diagonal matrix for with *"somewhat"* large values and $\mat{Q}$ should be a diagonal matrix with small values. Though, what is *"somewhat"* large and what is small dependent on the data set
...
As the variance $\mat{H}_t(\vec{\alpha}_t)$ and expected mean $h(\eta)$ are bound this means that all the terms are on a reasonably stable scale for all values of the linear predictor $\vec{\alpha}_t^T\vec{x}_{it}$
...
We will return to the vertical shortly. First, we notice that the example is where we assume that the covariates are step function. An example hereof could be a medical trial where patients get test taken at different point in time (when they have a time at their doctor, visit the hospital or similar). Ideally we would like to model that we know that for example individual a has the covariates from 0 to 1 and survive, the covaraites from 1 to 2 and survive etc.  
...
1. Parameters (that is state variables $\vec{\alpha}_1,\dots, \vec{\alpha}_d$) change at time $1, 2, \dots , d$
