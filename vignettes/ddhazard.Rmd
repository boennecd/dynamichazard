...
Step 0 comes at an $O(q^3)$ cost per interval $1,2,\dots,d$ due to the  Cholesky decomposition. This is durable if we do not have too many coeffecients. Further, a stable way to find the Cholesky decomposition is first to find the Cholesky decomposition of $\emNotee{\mat{V}}{t}{t-1}$ and then to perform the inversion. Step 2 can be performed in $O(q^2)$. Ideas for methods of doing the rank-one update can be found in [@gill74], [@seeger04] or at https://dahtah.wordpress.com/2011/11/29/rank-one-updates-for-faster-matrix-inversion/. Step 3 can be done with back substitution at a of cost $O(q^2)$. Thus, we end with an algorithm that is scale as weel as with the Woodbury matrix identity a part from the first step 0

Moreover, we can save computations throughout by storing $\widetilde{\mat{L}} = \Lparen{\mat{L}^{-1}}^\top$ instead of storing $\emNote{\mat{V}}{t}{t}{k}$. $\widetilde{\mat{L}}$ is also a triangular matrix. Thus, we need to do less operations when doing the matrix multiplications. Another advantage is that the rank-one update yields a positive definite matrix $\mat{L}\mat{L}^\top$. This is true since $\vec{x}_{ij}\vec{x}_{ij}^\top$ is a vector outer product and as $g$ is postive with distributions from the exponential family. Hence, $\emNote{\mat{V}}{t}{t}{k} = \Lparen{\mat{L}^{-1}}^\top\Lparen{\mat{L}^{-1}}$ will remains a positive definite matrix
