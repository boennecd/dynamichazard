...
# Intro
This note will cover the `ddhazard` function used for estimation in the `dynamichazard` library. You can install the version of the library used to make this vignettes from github with the `devtools` library as follows:
...
The `ddhazard` function estimates a binary regression model where the parameters are assumed to time-variant and follow a pre-defined random walk. 

## Why and when to use this package 
The package is implemented for situation where you have a binary regression model with time-varying coefficients. Although, it can be any natural ordering of observations other than time. The advantage of the state spaces methods used here is that you can extrapolate to time periods beyond the data used in estimation. An example is from my own research which is the topic of forecasting firm failures in the future given their present accounting data. The task is to use the present data (default and accounting data up to 2016) and forecast the likelihood of default for the firms in the following year. Another use of this package is as an alternative to other methods of modelling time-varying coefficients for binary regression
...
All of them are important for the research I am doing in firm failures. Firstly, you can easily have 40-50.000 firms at risk at each point in time. Thus, point 1) is key to be able to fit the models. Moreover, the number of firms at risk will vary as time proceeds. The causes are defaults, new firms or exits for other reason (mergers, acquisitions, voluntary liquidation etc.). This relates to point 2)

## Guide to vignettes 
The vignette here is the primary vignette which introduces the models and estimation methods. The other vignettes are supplementary. "Simulation study with logit model" shows a simulation study where the methods in this package are compared to each other and to Generalized Additive models. "Comparing methods for time-varying logistic models" does the same but for real world data set. Both vignettes illustrate how to use the estimation function `ddhazard` and other functions in this package
...
# Kalman Filter
The standard Kalman filter is carried out by recursively doing two steps. This also applies for the EKF and UKF. Thus, this paragraph is included to introduce general notions. The first step is in the Kalman Filter is the *filter step* where we estimate $\emNotee{\vec{a}}{t}{t - 1}$ and $\emNotee{\mat{V}}{t}{t - 1}$ based on $\emNotee{\vec{a}}{t - 1}{t - 1}$ and $\emNotee{\mat{V}}{t - 1}{t - 1}$. Secondly, we carny out the *correction step* where we estimate $\emNotee{\vec{a}}{t}{t}$ and $\emNotee{\mat{V}}{t}{t}$ based on $\emNotee{\vec{a}}{t}{t - 1}$ and $\emNotee{\mat{V}}{t}{t - 1}$ and the observations. We then repeat the process until $t=d$
...
[@fahrmeir92] mentions that the correction step can be viewed as a single Fischer Scoring step (and hence a step in a Newton Raphson method). This motivates:
...
where $\delta$ is small like $10^{-9}$. The defaults are $l = 1$ and $\epsilon_{\text{NR}} = \infty$. The latter yield only a single iteration in the scoring step. Selecting $l < 1$ in case of divergence can resolve the issue. Thus, the `ddhazard` function will try learning rates of $l, l\zeta, l\zeta^2 \dots l\zeta^w$ for given $0<\zeta<1$. The process is stopped when we succeed to fit the model or $w$ is reach without succeeding in estimating the model. Further, while [@fahrmeir92] does not observe improvements with multiple repetitions, we find improvements in terms of mean square error of the state vector by taking multiple steps (for example by setting $\epsilon_{\text{NR}} = 10^{-2}$ or lower) (see the vignette "Simulation study with logit model")
...
All the computations use objects from the `Armadillo` library. Thus, an optimized version LAPACK and BLAS can speed up the computation. A multithreaded version of LAPACK or BLAS can cause issues with performance. The majority of the computation time is spend in the correction step of the EKF when have to compute $\vec{u}_t (\vec{\alpha}_t)$ and $\mat{U}_t (\vec{\alpha}_t)$ when the number of regression parameter is low and we have a lot of observations. For this reason, this part of the code is computed in parallel with the `C++` standard library `thread`. The reduction in computation time can mitigated if multithreaded version of LAPLACK or BLAS is used as the code already use multithreading
...
The UKF selects state vectors called *sigma point* with given *sigma weigths* such that to match the moments of  observational equation. Thus, we approximate the density rather than approximating the observational equation. In essence, the idea like an Monte Carlo method for state space models but where the state vectors are chosen deterministically rather than randomly drawn

The motivation to use the UKF in place of the EKF as we avoid linerization error in the EKF. [@julier97] introduces a UKF that approximate the first two moments and up to fourth moment in certain settings. [@julier04] further develops the UKF and extend to what is later referred to as *the Scaled Unscented Transformation*. We will cover the the Scaled Unscented Transformation with the paramatization from [@wan00] and formulas from [@menegaz16]. The motivation for this choice is given at the end

Another motivation is that you do not have derive the Jacobian matrix. This is one the reason why the UKF has received a lot of attention in engineering where the observation equation is often complicated. However, deriving the Jacobian for the models in this package is not hard

## Usual UKF formulation
We start by introducing a common notation used in the UKF literature. Let:
...
where $\diag{\cdot}$ returns a diagonal matrix with the passed vectors values in the diagonal and $\vec{1}$ is a vector with one in all the entries. Next, we re-generate the sigma points by:
...
The above formulation of the UKF is from [@menegaz16]. I found this article useful for three reasons. 

1) The author gives an overview of the main UKF methods introduced in [@julier04], [@julier97], [@julier02reduced], [@julier03], [@wan07], [@menegaz11], [@wan00], [@lerner02] and [@charalampidis11]
2) The author shows that the above formulation is what the author coins as consistent. That is, it yields the same estimate as a regular Kalman filter when you use the method on a linear Gaussian state space model
3) They provide a simulation study for a non-linear system where the above method yields the best result in terms of mean square error for the state space vectors
...
This is what we approximately find the expectation of in the E-step of the EM-algorithm where $\mat{Q}$, $\mat{Q}_0$ and $\vec{\alpha}_0$ are assumed fixed. Denote the entries of the vector and the two matrices by $\vec{\theta}$. Then the M-step proceeds by maximizing $\vec{\theta}$ conditional on the estimates in the E-step. Expanding the unconditional log likelihood yields:  
...
The individuals covariates are simulated from a uniform distribution on $[0,1]$. The link function is the exponential model suggested above. Each bin has length 1 and we stop observing at time `r arg_list$t_max`
