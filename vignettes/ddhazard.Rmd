...
## UKF
TODO: Re-write to ridge regression like solution

This is not the case for the UKF method and the logistic model. We scale by $\wmat{H}^{-1}$ when we compute $\mat{G}$ and $\tilde{\vec{y}}$ which will approach infinity as linear predictor gets large in absolute value. For this reason, we truncate the linear predictor $\eta_{it} = \vec{\alpha}_t^T\vec{x}_{it}$ such that the variance cannot be less than some pre-specified quantity $\delta$. Effectively this means that we set:
$$\begin{aligned}
...
## Fixed effects
Fixed effects in the M-step is estimated like a regular GLM with a logistic link function where each individuals' observation will get an observation in the new model with an individual offset for each bin the observation is in. Say for instance that we want to fit a model to the following data frame: 
```{r, echo=FALSE}
set.seed(1010101012)
data_frame <- 
...
Thus, we have the following relation (conditional on having survived up the this point):
...
We could implement the above which is done that for the EKF. There is and issue though with divergence. The cause it that the entries of $\mat{K}$ can become numerically large. In particular, we can show that: 
...
We can compute some numerical examples to illustrate why this is an issue. This is done below. `eta` refers to the linear predictor, `delta` refers to at the at risk length and `Delta` refers to the truncated observation time:

```{r, echo=FALSE}
grid <- expand.grid(delta = c(.1, 1, 2), eta = c(-4, -2, 0))
...
We see that certain combinations of $(\delta, \eta)$ cause the entries of $\mat{K}$ (column K11, K12, K22) to be large. We will illustrate why this may be an issue in the following paragraphs. Notice that the derivatives of the two link functions (the entries under `Deriv ...`) have opposing signs while all the entries of $\mat{K}$ are positive. Thus, there is a canceling effect in the computation of $\mat{U}_t(\vec{\alpha}_t)$. Hence, $\emNotee{\mat{V}}{t}{t}$ seems not to be an issue in the sense that it "stays around" sensible values before divergence. The update from $\emNotee{\vec{a}}{t}{t-1}$ to $\emNotee{\vec{a}}{t}{t}$ seems to be the cause divergence which we will illustrate in the following paragraphs. Re-call that:
...
Suppose now that we have somewhat rare events, the linear predictor $\eta$ is `r grid$eta[ex]`, $\delta$ is `r grid$delta[ex]`  and the individual with $\Delta = 1$. The latter can occur often if times are reported discreetly (in say days or months). Then the factor from the residual in the term  with $(y_i - h_i^y(\eta_i))$ is the product of `r grid[ex, "Deriv link Y"]` ($\Lparen{h_i^y(\eta_i)}'$), (`r grid[ex, "K11"]` plus `r grid[ex, "K12"]`) ($\MyInd{\mat{K}_{11}}{ii} + \MyInd{\mat{K}_{12}}{ii}$) and ($y_i$ minus `r grid[ex, "Link Y"]` ($h_i^y(\eta_i)$)). The result is `r grid[ex, "Deriv link Y"] * (grid[ex, "K11"] + grid[ex, "K12"]) * (1 - grid[ex, "Link Y"])` if it is a death and `r grid[ex, "Deriv link Y"] * (grid[ex, "K11"] + grid[ex, "K12"]) * (0 - grid[ex, "Deriv link Y"])` otherwise

The factor for the residual time is the product of `r grid[ex, "Deriv link Delta"]`, (`r grid[ex, "K22"]` plus `r grid[ex, "K12"]`) and (1 minus `r grid[ex, "Link Delta"]`). The result is `r grid[ex, "Deriv link Delta"] * (grid[ex, "K22"] + grid[ex, "K12"]) * (1 - grid[ex, "Link Delta"])`. Meanwhile the factor for the information matrix $\mat{U}_t(\vec{\alpha})_t$ is `r grid[ex, "Deriv link Y"]^2 * grid[ex, "K11"] + 2 * grid[ex, "Deriv link Y"] * grid[ex, "Deriv link Delta"] * grid[ex, "K12"] + grid[ex, "Deriv link Delta"]^2 * grid[ex, "K22"]` which is computed by  $\Lparen{\Lparen{h^y_i}'(\eta_i)}^2\MyInd{\mat{K}_{11}}{ii} + 2\cdot\Lparen{h^y_i}'(\eta_i)\cdot\Lparen{h^\Delta_i}'(\eta_i)\MyInd{\mat{K}_{12}}{ii} + \Lparen{\Lparen{h^\Delta_i}'(\eta_i)}^2\MyInd{\mat{K}_{22}}{ii}$. Recall that:

$$\emNotee{\mat{V}}{t}{t} = \left(\emNotee{\mat{V}}{t}{t - 1}^{-1} + \mat{U}_t (\emNotee{\vec{a}}{t}{t - 1})\right)^{-1}$$
Depending on the co-varaite vectors $\vec{x}_i$ and covariance matrix $\emNotee{\mat{V}}{t}{t - 1}$ this can imply that $\vec{u}_t(\vec{\alpha}_t)$ becomes large compared to $\emNotee{\mat{V}}{t}{t }$ which causes a large step in the score direction. 

<!-- (One solution is to set the learning rate $l$ (very) low. Though, I have to stress that this has to be set it at a very low level)

Another solution may be to make the two variables uncorrelated. For example, we can set:
$$\begin{aligned}
...
Though, this requires that we know the $\vec{\alpha}_t$ in order to compute $y_i$. Question: Would it be ok to plug in the current estimate of $\emNotee{\vec{a}}{t}{t - 1}$? Do you think this is a good idea? Issue: The resultning variable has very low variance and thus does not resolve the problem. --> 

Two solutions are: 1) decrease the learning rate until we get a fit or 2) notice the entries of the inverse covariance matrix, $\mat{K}$, are large due the division by $1-\rho_i^2$. Hence, scale $\mat{K}$ by $1-\rho^2_i$ to get:

$$\mat{K} = \begin{pmatrix}
  \diag{\frac{1}{\sigma_{Y_1}^2}, \dots, \frac{1}{\sigma_{Y_r}^2}} &
    \diag{\frac{-\rho_1}{\sigma_{Y_1}\sigma_{\Delta_1}}, \dots, \frac{-\rho_r}{\sigma_{Y_r}\sigma_{\Delta_r}}} \\
  \diag{\frac{-\rho_1}{\sigma_{Y_1}\sigma_{\Delta_1}}, \dots, \frac{-\rho_r}{\sigma_{Y_r}\sigma_{\Delta_r}}} &
    \diag{\frac{1}{\sigma_{\Delta_1}^2}, \dots, \frac{1}{\sigma_{\Delta_r}^2}}
\end{pmatrix}$$

with
$$\frac{-\rho_i}{\sigma_{Y}\sigma_{\Delta}} = 
  -\frac{\eta  \exp\Lparen{-\delta  \eta } \left(\exp\Lparen{\delta  \eta } (1-\delta  \eta )-1\right)}{\left(1-\exp\Lparen{-\delta 
   \eta }\right) \left(-2 \delta  \eta  \exp\Lparen{-\delta  \eta }-\exp\Lparen{-2 \delta  \eta }+1\right)}$$

```{r, echo = FALSE}
source("../R/test_utils.R")
n_sims <- 100
arg_list <- list(n_series = 1e3, n_vars = 5, t_0 = 0, t_max = 10,
                 x_range = 1, x_mean = 0, re_draw = T, beta_start = c(-2:2) / 2,
                 intercept_start = -3, sds = c(.1, rep(1, 5)))
```

where we suppressed the subscript on the right hand site for. We will run the following simulations to test the difference between using the true inverse covariance or the suggested scaled inverse covariance. In short, we try to fit the true model. We simulate `r n_sims` runs each with `r arg_list$n_series` individuals. We have `r arg_list$n_vars` covariates starting at `r arg_list$beta_start[1:4]` and `r arg_list$beta_start[5]` and all being independent with a standard deviation of `r arg_list$sds[2]`. Moreover, the intercept starts at `r arg_list$intercept_start` with a standard deviation of `r arg_list$sds[1]`. An example of the covariates in one simulation is given in a plot below. The black line s the intercept: 

```{r, echo = FALSE, message=FALSE, warning=FALSE}
set.seed(44444314)
sims <- do.call(test_sim_func_exp, arg_list)

matplot(seq_len(nrow(sims$betas)) - 1, sims$betas, 
                type = "l", lty = 1, xlab = "Time", ylab = "Coeffecient")
```

The individuals covariates are simulated from a uniform distribution on $[0,1]$. We use the continous time model suggested above. Each bin has length 1 and we stop observing at time `r arg_list$t_max`
...
dd1_args <- list(
  ...
  Q_0 = diag(1, 6),
  Q = diag(1e-2, 6),
  control = list(est_Q_0 = F, eps = 10^-1, n_max = 10^3, LR_max_try = 20),
  max_T = 10,
  id = sims$res$id, order = 1,
  model = "exponential")

dd2_args <- dd1_args
dd2_args$control$EKF_inv_Cov_method <- "not org"

out <- array(NA_real_, dim = c(n_sims, 2, 2),
              dimnames = list(NULL, c("Normal inv covariance", "Altered inv covariance"),
                              c("In sample Brier score", "Learning rate")))
...
for(i in 1:n_sims){
  repeat{
    ...
    if(sum(sims$res$event) > 50 && arg_list$n_series - sum(sims$res$event) > 50)
      break
  }
  
  dd1_args$data <- sims$res
  dd1_args$id <- sims$res$id
  f1 <- do.call(ddhazard, dd1_args)
  
  dd2_args$data <- sims$res
  dd2_args$id <- sims$res$id
  f2 <- do.call(ddhazard, dd2_args)
  
  # matplot(sims$betas, type = "l", lty = 1)
  # matplot(f1$state_vecs, type = "l", lty = 2, add = T)
  # matplot(f2$state_vecs, type = "l", lty = 4, add = T)
  
  dat_frame <- get_survival_case_weigths_and_data(
    dd1_args$formula, dd1_args$data, 
    max_T = dd1_args$max_T, by = dd1_args$by, id = dd1_args$id, use_weights = F)
  
  out[i, "Normal inv covariance", "In sample Brier score"] <-
    mean.default((dat_frame$Y - predict(f1, new_data = dat_frame, tstart = "tstart", tstop = "tstop")$fits)^2)
  out[i, "Altered inv covariance", "In sample Brier score"] <-
    mean.default((dat_frame$Y - predict(f2, new_data = dat_frame, tstart = "tstart", tstop = "tstop")$fits)^2)
  
  out[i, "Normal inv covariance", "Learning rate"] <- f1$LR
  out[i, "Altered inv covariance", "Learning rate"] <- f2$LR
}
```

The result of the simulations are printed in the table below. We report the in sample Brier score, standard deviation of the Brier score and mean and median learning rate (it is decreased until we get a fit). It seems that there is little difference between the two in terms of Brier score. However, the used learning rate is higher for the method where we have scaled up the element of the inverse covariance matrix

```{r, echo = FALSE, message=FALSE, warning=FALSE}
tbl <- matrix(NA_real_, nrow = 2, ncol = 4, 
              dimnames = list(
                dimnames(out)[[2]], c("Mean in-sample Brier score", "Sd in-sample Brier score", "Mean learning rate", "Median learning rate")))

tbl[, "Mean in-sample Brier score"] <- colMeans(out[, , "In sample Brier score"])
tbl[, "Sd in-sample Brier score"] <- apply(out[, , "In sample Brier score"], 2, sd)
tbl[, "Mean learning rate"] <- colMeans(out[, , "Learning rate"])
tbl[, "Median learning rate"] <- apply(out[, , "Learning rate"], 2, median)

knitr::kable(tbl[, 1:2], digest = 3)
knitr::kable(tbl[, 3:4], digest = 3)
```

As a final example, here is the output from the E-step of the EKF for the head neck data in [@fahrmeir94]. You can get similar output by passing `control = list(debug = T, ...)` to the `ddhazard` call. Be aware that you will get a lot of output so it is likely helpful to use `sink` to write output to a file.  `U` is the diagonal of the information matrix $\mat{U}$:
...
The starting value here is not too far off from the what we would expect. The reason is that only one individual dies in the first bin. The logit model is estimated to start of at roughly $\vec{\alpha}_0 = (-3, 0)$. Thus, we would expect something similar here due to the link between the cloglog function and the logit function. Below is the output from the continous time model with a learning rate of 0.1 (it is rather long). Both the final state vector and smoothed covariance matrices match well with the logistic:
...
U
    7.9229     4.5404 
a_(2|2)
   -3.2088    0.32334 # <-- This is more reasonable
diag(V_(2|2))
...
[OMITTED OUTPUT FOR NEXT ITERATIONS UNTIL THE FINAL]
...
## EKF
The EKF method for the continous time model has shown to have issues with divergence. The cause seem to be overstepping in the correction step. Hence, we can set the starting value for the learning rate below 1. For example, you can do so by passing `list(LR = .5, ...)` to the `control` argument of `ddhazard` to gain a learning rate of $0.5$

The direct formulas for the conditional mean, conditional variance and derivatives of the conditional mean are numerically unstable for certain combinations of interval lengths ($t_{i,s} - t_{i, s - 1}$) and the linear predictor ($\eta = \vec{x}_{is}^T \vec{\alpha}_{\Lceil{t_{i,s}}}$). In particular they are subject to cancellation. This can cause both high relative errors and even wrong sign in some of the cases. Thus, Lauren series or Taylor series are used for numerically unstable computation. This requires no action for user but is stressed for readers who worry after seeing both the conditional mean and conditional variance of $\Delta_{i,s}$

## UKF
The implementation is like the implementation of the UKF for the logit model. We have a similar issue when $\widehat{\mat{H}}^{-1}$ approach infinite for certain settings of at risk length and linear predictor. For simplicity of notation, denote the risk length of a given observation by $\delta$ and the linear predictor by $\eta$. Then we solve the issue by left or right truncating $\eta$ such that the probability of dying is bounded. That is, we set $\eta$ such that: 
$$\epsilon \leq 1 - \exp \Lparen{-\exp(\eta)\delta} \leq 1 - \epsilon$$

For a given threshold $\epsilon\in[0, 1/2)$. The user will be warned if the threshold is passed at least once
...
Then individual 1 will yield five observations. The first row would only has an offset of $\log 0.5$ plus $\emNotee{\vec{a}}{1}{d}$ times `r data_frame_new$x1[1]`. The second row will yield two observations: one with an offset of $\log 0.5$ plus $\emNotee{\vec{a}}{1}{d}$ times `r data_frame_new$x1[2]` and the other with an offset $\log 1$ plus $\emNotee{\vec{a}}{2}{d}$ times `r data_frame_new$x1[2]`. Note that this is not the case with the logistic model as we have the same binning issue as described in the Binning section
