...
This is in the typical start and stop time format. The column `id` shows which individual the row belongs to, `tstart` is point at which the row is valid from and `tstop` is when the row is valid to. `event` is one if the individual dies at `tstop` and `x1` and `x2` are two covariates. Thus, the individual with id `1` dies at time `r simple_ex$tstop[1]` while id `2` survives all the periods we observe. The models we look at will allow for censoring to deal handle an individual like id `2`
...
$$\begin{aligned}
 \proppCond{Y_{ijt} = 1}{\vec{y}_{1},\dots,\vec{y}_{t-1}, \vec{\alpha}_t} = h(\vec{\alpha}_t^T \vec{x}_{ijt})
\end{aligned}$$
...
$$\begin{aligned}
  z_{ft}(\vec{\alpha}_t) &=\expecpCond{Y_{ijt}}{\vec{\alpha}_t} = h(\vec{\alpha}_t^T \vec{x}_{ijt}) \\
...
where $\mat{0}_m$ is a $m\times m$ matrix with zeros in all entries. The vector in the state equation is ordered as $\vec{\alpha}_t = (\wtvec{\alpha}_t^T, \wtvec{\alpha}_{t-1}^T)^T$ to match the definition of $\mat{F}$ and $\mat{R}$ where the tilde is added to indicate the coefficients used when computing the linear predictor. The likelihood of the model where $\vec{\alpha}_t$ are observed can be written as follows by application of the markovian property of the model:
$$\begin{split}
...
$$l_{ijt}(\vec{\alpha}_t) = y_{ijt} \log h(\vec{x}_{ijt}^T \vec{\alpha}_t) + (1 - y_{ijt}) 
	\log \Lparen{1 - h(\vec{x}_{ij}^T \vec{\alpha}_t)}$$

The unknown parameters are the initial state vector $\vec{\alpha}_0$ and the covariance matrix $\mat{Q}$ . We estimate these using an EM-algorithm. The E-step is carried out first by filtering with an Extended Kalman filter (EKF), an Unscented Kalman filter (UKF) or an approximation of the posterior modes. We apply a smoother after the filtering. The method is chosen by passing a list to the `control` argument of `ddhazard` with `list(method = "EKF", ...)`, `list(method = "UKF", ...)` or `list(method = "post_approx", ...)` respectively. All filtering methods requires an initial state vector $\vec{\alpha}_0$, co-variance matrix $\mat{Q}$ and initial co-variance matrix $\mat{Q}_0$ to start
...
$$\begin{aligned}
  & \vec{u}_t (\vec{\alpha}_t) = \sum_{(i,j) \in R_t} \vec{u}_{ijt} (\vec{\alpha}_t), \quad\vec{u}_{ijt} (\vec{\alpha}_t)= \left. \vec{x}_{ij} \frac{\partial h(\eta)/ \partial \eta}{H_{fft}(\vec{\alpha}_t)} \Lparen{y_{ijt} - h(\eta)} \right\vert_{\eta = \vec{x}_{ij}^T \vec{\alpha}_t} \\
%
	& \mat{U}_t (\vec{\alpha}_t) = \sum_{(i,j) \in R_t} \mat{U}_{ijt} (\vec{\alpha}_t), \quad \mat{U}_{ijt} (\vec{\alpha}_t) = \left. \vec{x}_{ij} \vec{x}_{ij}^T 
		 \frac{\left( \partial h(\eta)/ \partial \eta \right)^2}{H_{fft}(\vec{\alpha}_t)} \right\vert_{\eta = \vec{x}_{ij}^T \vec{\alpha}_t}
\end{aligned}$$
...
$$\begin{aligned}
  & \vec{u}_t (\vec{\alpha}_t) = \sum_{(i,j) \in R_t} \vec{u}_{ijt} (\vec{\alpha}_t), \quad\vec{u}_{ijt} (\vec{\alpha}_t)= \left. \vec{x}_{ijt} \frac{\partial h(\eta)/ \partial \eta}{H_{fft}(\vec{\alpha}_t) + \xi} \Lparen{y_{ijt} - h(\eta)} \right\vert_{\eta = \vec{x}_{ijt}^T \vec{\alpha}_t} \\
%
	& \mat{U}_t (\vec{\alpha}_t) = \sum_{(i,j) \in R_t} \mat{U}_{ijt} (\vec{\alpha}_t), \quad \mat{U}_{ijt} (\vec{\alpha}_t) = \left. \vec{x}_{ijt} \vec{x}_{ijt}^T 
		 \frac{\left( \partial h(\eta)/ \partial \eta \right)^2}{H_{fft}(\vec{\alpha}_t) + \xi} \right\vert_{\eta = \vec{x}_{ijt}^T \vec{\alpha}_t}
\end{aligned}$$
where $\xi>0$ is a small number. The default can be changed by altering the `ridge_eps` in the list passed to the `control` argument of `ddhazard`. The approach is similar to how the `glmnet` package handles close to boundary estimates (see [@friedman10])
...
$$\begin{aligned}
  ...
  \wvec{a}_{j} &= \emNotee{\vec{a}}{t}{t-1} + \sqrt{q + \lambda} \left(\sqrt{\emNotee{\mat{V}}{t}{t - 1}}\right)_j \\
  \wvec{a}_{j + q} &= \emNotee{\vec{a}}{t}{t - 1} - \sqrt{q + \lambda} \left(\sqrt{\emNotee{\mat{V}}{t}{t - 1}}\right)_j
...
$$\begin{aligned}
  \wvec{y}_j &= \vec{z}_t \left(\wvec{a}_j \right), \qquad j = 0,1,\dots, 2q \\
...
$$\begin{aligned}
  ...
  \emNotee{\mat{V}}{t}{t} &= \emNotee{\mat{V}}{t}{t-1} - \ukfNotee{\alpha}{y}{t}\ukfNotee{y}{y}{t}^{-1}\ukfNotee{\alpha}{y}{t}^T
\end{aligned}$$
...
where $\tilde{\vec{y}}$, $\mat{G}$, $\mat{L}$ and $\vec{c}$ are intermediates. The above algorithm is $O(n_t)$ since $\widehat{\mat{H}}$ is a diagonal matrix and all products involves at most multiplications of $q\times n_t$ or $n_t \times q$ matrices
...
# Approximation of the posterior mode
...
Further, we can apply Woodbury matrix identity to avoid the inversions and get:
...
This method is selected by setting `method = posterior_approx` in the list passed to the `control` argument of `ddhazard`

## Implementation 
Finding the constant $v$ in step can be done by the Newton Raphson method to find a unique minimum when: 

1. $- \Lparen{y_{ijt} \log h(b) + (1 - y_{ijt}) \log (1 - h(b)}$ is convex in $b$
2. $- \Lparen{y_{ijt} \log h(b) + (1 - y_{ijt}) \log (1 - h(b)}$ has a finite minimum

Number 1. and 2. are true for binomial model and true for all exponential families where the form though will change. That is, we have to change the likelihood expression of $-\log \proppCond{y_{ijt}}{\vec{\alpha}}$. 2. may further be at a limit as is the case for the binomial models since for $y_{ijt} = 1$ the limit is $0$ at $b \rightarrow \infty$ and for $y_{ijt} = 0$ the limit is $0$ for $b \rightarrow -\infty$. This is true since the convex polynomial will eventually dominate the slope of the logit function which tends towards zero. Further, the learning rate $\zeta_0$ the decrease factor $\zeta$ in the EKF can be used here to by changing the correction step to:
...
\notaTbl{
  ...
  Scalability & This method is also $O(n_t)$ when we use the Woodbury matrix identity to update the covariance matrix
}
...
$$\tilde{\vec{y}} = \Delta \wmat{Y}^T \widehat{\mat{H}}^{-1}(\vec{y}_t - \overline{\vec{y}}) \qquad
    \mat{G} = \Delta\wmat{Y}^T\widehat{\mat{H}}^{-1}\Delta\wmat{Y}$$
    
with

$$\tilde{\vec{y}} = \Delta \wmat{Y}^T \mat{E}_t\widehat{\mat{H}}^{-1}(\vec{y}_t - \overline{\vec{y}}) \qquad
    \mat{G} = \Delta\wmat{Y}^T\mat{E}_t\widehat{\mat{H}}^{-1}\Delta\wmat{Y}$$

## Posterior mode approximation
Weights are handled in the posterior mode approximation by replacing: 
$$\begin{aligned}
...
Suppose now that we assume that some of the effects are fixed such that we replace the linear predictor $\vec{x}_{it}^T\vec{\alpha}_t$ by $\wtvec{x}_{ij}^T\widetilde{\vec{\alpha}}_t + \bvec{x}_{ij}^T\vec{\gamma}$ where $\vec{\gamma}$ is the fixed coefficients and $\bvec{x}_{it}$ are the corresponding covariates. The new definition of $l_{ijt}$ is:

$$l_{ijt}(\widetilde{\vec{\alpha}}_t, \vec{\gamma}) = y_{ijt} \log h(\wtvec{x}_{ij}^T \widetilde{\vec{\alpha}}_t + \bvec{x}_{ij}^T\vec{\gamma}) + (1 - y_{ijt}) 
	\log \Lparen{1 - h(\wtvec{x}_{ij}^T \widetilde{\vec{\alpha}}_t + \bvec{x}_{ij}^T\vec{\gamma})}$$
...
1. The $\bvec{x}_{ij}^T\vec{\gamma}^{(k-1)}$ term acts like offsets in the E-step where $\vec{\gamma}^{(k-1)}$ is fixed. Thus, we only need to add these offsets to the linear predictors
...
$$\xyzp{\widetilde{E}_k}{ \sum_{t = 1}^d \sum_{(i,j) \in R_t} l_{ijt}(\wtvec{\alpha}_t, \vec{\gamma})}$$

with respect to $\vec{\gamma}$. The update formulas are not as simple as for $\wtvec{\alpha}_0^{(k)}$ and $\mat{Q}^{(k)}$ as the terms of $l_{ijt}$ are non-linear in the time-varying effects $\wtvec{\alpha}_0\dots,\wtvec{\alpha}_d$. A simple way to overcome this is to make a zero order Taylor expansion around the mean estimates $\emNote{\wtvec{a}}{0}{d}{k},\dots,\emNote{\wtvec{a}}{d}{d}{k}$:
$$\xyzp{\widetilde{E}_k}{ \sum_{t = 1}^d \sum_{(i,j) \in R_t} l_{ijt}({\wtvec{\alpha}_t}, \vec{\gamma})}
  \approx \sum_{t = 1}^d \sum_{(i,j) \in R_t} l_{ijt}(\emNote{\wtvec{a}}{t}{d}{k}, \vec{\gamma})$$
  
This expansion coincides with a first order Taylor expansion as the first order terms are zero. The advantages are:

3. $\wtvec{x}_{ij}^T \wtvec{a}_t$ acts like offsets in the M-step when we estimate $\vec{\gamma}^{(k)}$
...
### Other options
Another option is to use higher order expansions of $\xyzp{\widetilde{E}_k}{ \sum_{t = 1}^d \sum_{(i,j) \in R_t} l_{ijt}({\wtvec{\alpha}_t})}$, approximate the expectation with an MCMC method using the conditional means $\emNote{\wtvec{a}}{0}{d}{k},\dots,\emNote{\wtvec{a}}{d}{d}{k}$ and conditional covariance matrices $\emNote{\mat{V}}{0}{d}{k},\dots,\emNote{\mat{V}}{d}{d}{k}$, or any other method to approximate $\xyzp{\widetilde{E}_k}{ \sum_{t = 1}^d \sum_{(i,j) \in R_t} l_{ijt}({\wtvec{\alpha}_t})}$. At this point, the zero order Taylor expansion is the only implemented method to estimate $\vec{\gamma}$ in the M-step
...
3. We have piecewise constant instantaneous hazards given by $\exp(\vec{x}_{ij}^T\vec{\alpha}_t)$ given an individual's current co-covariate vector $\vec{x}_{ij}$ and state variable $\vec{\alpha}_t$ (assuming that individual $i$'s $j$'th covariate is within time interval $t$)
...
%
  l_{ijt}(\vec{\alpha}_t) =& (\vec{x}_{ij}^T\vec{\alpha}_t)^{y_{ijt}} -\exp\Lparen{\vec{x}_{ij}^T\vec{\alpha}_t}
...
$$\begin{aligned}
  \tilde{z}(\vec{\alpha}) &= \expecpCond{\Delta_{ijt}}
    ...
    = h^{\Delta}_{ft}(\vec{x}_{ij}^T\vec{\alpha}_t) \\
...
where $f$ is the index of observational equation at time $t$ matching with individual $i$'s $j$'th covariate vector, $\Lceil{t_{ij}}=s$ is the time interval number that the observation is in, $\tilde{T} \sim \text{Exp}\Lparen{\exp(\vec{x}_{ij}^T\vec{\alpha}_t)}$, $f_{\tilde{T}}$ is the density function of $\tilde{T}$ and $h^{\Delta}_{fj}$ is the inverse link function for the right clipped time variables for individual $i$'s $j$'th observation. Set $\lambda = \exp(\vec{x}_{ij}^T\vec{\alpha}_t)$ and $\delta = \delta_{ijt}$. The resulting conditional mean is:

$$h^{\Delta}_{ft}(\vec{x}_{ij}^T\vec{\alpha}_t) = \frac{1 - \exp \Lparen{- \lambda \delta} }{\lambda}$$
...
$$l_{ijt}(\vec{\alpha}_t) = y_{ijt} \log h^Y_{ft}(\vec{x}_{ij}^T \vec{\alpha}_t) + (1 - y_{ijt}) 
	\log \Lparen{1 - h^Y_{ft}(\vec{x}_{ij}^T \vec{\alpha}_t)}$$
	
where the inverse link function is the inverse cloglog function. That is,
$$\begin{aligned} 
h^Y_{ft}(\vec{x}_{ij}^T \vec{\alpha}_{t}) =  1 -  \exp\Lparen{ - \exp(\vec{x}_{ij}^T \vec{\alpha}_{t}) 
    \Lparen{\min\{t, t_{ij} \} - \max\{ t - 1, t_{i,j-1} \} }}
\end{aligned}$$
...
There are two points to be made here. Firstly, we do not round up by using $\bar t_{ij}$ in place of $t_{ij}$ when we have a death. Thus, we do take into account the moment the person dies at in that $\log h^Y_{ft}(\vec{x}_{ij}^T \vec{\alpha}_t)$ is the likelihood of dying sometime in the period $\delta_{ijt}$ given that he survived up to $t_{i,j-1}$. Moreover, an individual can have multiple observations in the same time interval if he does not die but his covariate vectors change within the interval in the state space model. Thus, we do not have issue with going from event times to binary outcomes as with the logistic model 

The con is that the term $\log h^Y_{ft}(\vec{x}_{ij}^T \vec{\alpha}_t)$ is only the log likelihood of dying *sometime* in the period $\delta_{ijt}$. It is not the likelihood of dying after *exactly* $\delta_{ijt}$. We do not loose this information with the right clipped observation time. This may be a minor drawback in settings where we have interval censoring. Here the exact time of death may be unknown and the given stop time is an upper bound for the death time. You can estimate with the binary outcome method by setting the argument `model = "exp_bin"` in the call to `ddhazard`
...
\Lambda_{ijt} 
  ...
  &= \left\{\begin{matrix}
    ...
    \delta_{ijt} & T_i > \min\Lbrace{t_{ij},t}
\end{aligned}$$
...
$$h_{fs}^\Lambda(\vec{\alpha}) = \frac{\left(1 - \exp\Lparen{-\delta  \lambda }\right) (\delta  \lambda
   -1)}{\lambda }$$
where the definition of $\delta$ and $\lambda$ is the same as in the right clipped time variable section. I.e. we are looking at a particular observation for individual $i$ and define $\lambda = \exp(\vec{x}_{ij}^T\vec{\alpha}_t)$ and $\delta = \delta_{ijt}$. The mean is plotted below as a function of $\lambda$ with $\delta = 1$
...
Another idea is to use the binary variable and the first mentioned right clipped time at the same time. Though, this method has shown worse performance on simulated data. Thus, the details of the method is omitted
...
$$\begin{aligned}
  & \hat{y}_{ijt} = \exp \left( \vec{x}_{ij}^T\emNotee{\vec{a}}{t}{d})\right) / \left( 1  + \exp\left(\vec{x}_{ij}^T\emNotee{\vec{a}}{t}{d})\right) \right) \\
%
...
## Hogwill
TODO: mention Hogwill, motivate why to use with binary/sparse features
...
\notaTbl{
	...
	$n_t = \vert R_t \vert$ & Dimension/number of elements of the risk set at time $t$ \\ \hline
	...
	$v,b,g,$ $\vec{v}, \vec{b}, \wtvec{y}, \vec{c},$ $\mat{A}, \mat{C},\mat{K}, \mat{G}, \mat{L}$ & Scalars, vectors and matrices which depend on the context \\ \hline
...
\notaTbl{
	$\wvec{a}_j$ & $j$'th sigma point \\  \hline
