...
The following section introduce the continous time model. Three different method will be introduced to estimate the model. We start by describing the assumption of the continous time model. Then we turn to each of the three estimation methods in turn
...
\mathcal{L}\Lparen{\vec{\alpha}_0, \dots, \vec{\alpha}_{d}} =&
  - \frac{1}{2} \Lparen{\vec{\alpha}_0 - \vec{a}_0}^T \mat{Q}^{-1}_0\Lparen{\vec{\alpha}_0 - \vec{a}_0} \\
	&  - \frac{1}{2} \sum_{t = 1}^d \Lparen{\vec{\alpha}_t - \mat{F}\vec{\alpha}_{t - 1}}^T \mat{Q}^{-1} \Lparen{\vec{\alpha}_t - \mat{F}\vec{\alpha}_{t - 1}} \\ 
	&  - \frac{1}{2} \deter{\mat{Q}_0} - \frac{1}{2d} \deter{\mat{Q}} \\
  &+ \sum_{s=1}^n\sum_{(i,j) \in \Lbrace{(i,j) \left\vert\begin{smallmatrix} t_{i,j-1} < s \\ t_{i,j} > s-1 \end{smallmatrix} \right.}}
  l_{i,j,s}(\vec{\alpha}_s) \\
%
  l_{i,j,s}(\vec{\alpha}_s) =& (\vec{x}_{i,j}^T\vec{\alpha}_s)}^{y_{i,j,s}}{\Lparen{-\exp\Lparen{\vec{x}_{i,j}^T\vec{\alpha}_s}
  \Lparen{\min\{ s, t_{i,j} \} - \max\{ s - 1, t_{i,j-1} \} }}
\end{aligned}$$

The following section will introduce three ways to model the above. They are:

 1. Using a right truncated time variable
 2. Using a binary variable
 3. Using the above two at the same time
 
The model with the binary outcome and the right truncated time variable seems to do well on simulated data. Hence, you may want to focus on these section and breifly read or skip the section on the combined model where we use both variables at the same time
...
where we suppress the dependents on $\vec{\alpha}_s$. Furhter, order the observational equation such that
...
You can estimate a with the tuplets method by [TODO: insert method you need to parse]
...
The method with the tuplets is more prone to divergece due to the correlation between the tuplets. Thus, a ridge regression like solution is used where $\mat{H}_s(\vec{\alpha}_t)$ is replaced by:
...
$$\widetilde{\widehat{\mat{H}}} = \widehat{\mat{H}} + \xi \mat{I}$$
The matrix inversion is easily caried out with the matrix since it only invovles inversions of $2\times 2$ matricies
...
In this section, we will make the same assumption and use the same notation as in EKF method for the continous time model with tuplets. The formulas are given for each of elements of the block diagonal matricies of the inverse covariance matrix with the ridge regression like term. That is, the block matricies of the inverse of $\widetilde{\mat{H}}_s(\vec{\alpha}_s) = \mat{H}_s(\vec{\alpha}_s) + \xi\mat{I}$
$$\tiny\begin{aligned} 
