...
Another option is to perform the E-step using an unscented Kalman filter. This is done below
```{r, fig.height = 6, mySettings=FALSE}
dd_fit_UKF <- ddhazard(Surv(tstart, tstop, death == 2) ~ age +
                   ...
                   control = list(method = "UKF", beta = 0, alpha = 1e-1, eps = .05), 
                   verbose = 5)
...
tmp_file <- file("tmp.txt")
sink(tmp_file)
dd_fit_UKF <- ddhazard(Surv(tstart, tstop, death == 2) ~ age +
                         edema + log(albumin) + log(protime) + log(bili), pbc2,
                   id = pbc2$id, by = 100, max_T = 3600, 
                   Q_0 = diag(rep(1 / 100, 6)), Q = diag(rep(0.01 / 100, 6)),
                   control = list(method = "UKF", beta = 0, alpha = 1, eps = .01, 
                                  debug = F), 
                   verbose = 5)
sink()
close(tmp_file)
...
# Diverges
tryCatch({
  dd_fit <- ddhazard(Surv(tstart, tstop, death == 2) ~ ddFixed(age) + ddFixed(log(albumin)) +
                          edema + log(protime) + log(bili), pbc2,
                     id = pbc2$id, by = 100, max_T = 3600, 
                     Q_0 = diag(rep(1, 4)), Q = diag(rep(0.01, 4)), 
...
dd_fit <- ddhazard(Surv(tstart, tstop, death == 2) ~ ddFixed(age) + ddFixed(log(albumin)) +
                          edema + log(protime) + log(bili), pbc2,
                     id = pbc2$id, by = 100, max_T = 3600, 
                     Q_0 = diag(rep(10, 4)), Q = diag(rep(0.01, 4)), 
                     control = list(eps = 0.01, LR = .6) # <-- we change the LR here
                   )
