...
Next, we plot the coefficient. Notice that the coefficient are close to the estimates we saw previously for the GAM model. A key thing to notice is that `Q_0` is not scaled by the length of the bins while `Q` is. Hence, `Q` should not change much if we alter the length of the interval. We can illustrate this by fitting a model with a shorter bin lengths:

```{r, warning=FALSE}
# First diagonal of the the current co-variance matrix estimate
(old_mat <- dd_fit$Q) * 1e4 # scale for a neater print
...
dd_fit <- ddhazard(Surv(tstart, tstop, death == 2) ~ age + edema +
                   ...
                   Q_0 = diag(rep(10, 6)), Q = diag(rep(0.001, 6)),
                   control = list(ridge_eps = .0001 / 2)) # <-- we decrease the penalty term by a similar factor
