...
1. The UKF may work for certain data set. It may require tuning for some data sets
...
Another method is to use the sequential rank-one approximation of the posterior posterior modes. This is done below. First, we estimate the model with the Extended Kalman filter and next with rank-one posterior modes approximation. We estimate both for comparison:
...
dd_fit_post_approx <- 
  ddhazard(Surv(tstart, tstop, death == 2) ~ age + edema +
           ...
           control = list(method = "post_approx"), # change estimation method 
                                                   # from default
           Q_0 = diag(rep(100000, 6)), Q = diag(rep(0.01, 6)))
```
...
for(i in 1:6){
  ...
  plot(dd_fit_post_approx, cov_index = i, col = "red", add = TRUE)
}
```
...
The fixed effects estimates are not to far from each other:
