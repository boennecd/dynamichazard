...
options(digist = 4)
knitr::opts_chunk$set(echo = TRUE, fig.height = 5, dpi = 144, 
                      dev = "png",
                      cache = T, 
...
This vignette will show how to bootstrap the confidence intervals of a `ddhazard` object. It is recommended to read or skim `vignette("ddhazard", "dynamichazard")` first. You can get the version used to make this vignette by calling:
...
# TRACE
We will use the `TRACE` data set from the `timreg` package. See `?TRACE` for details on the dataset and `?timereg::aalen` for a additive hazard model some of which are (kinda) like the one we fit next. The `TRACE` data set is used here to illustrate the bootstrap methods and not as example of how to analysis the data set. I have not looked at the details or model fits in detail. We fit the model as follows:
...
We use the exponential arrival times models with the EKF (the default) estimation method. A plot of the estimates is given below. The dashed lines are 95% point-wise confidence intervals using the variances estimates from the Extended Kalman filter with smoothing

```{r}
plot(dd_fit)
dd_fit$fixed_effects # the fixed effects estimates
dd_fit$Q
```
...
We may also want to get simultaneous confidence intervals. An easy way to get such confidence intervals is with the `envelope` function in the `boot` library. For instance, we can simultaneous confidence intervals for the `vf` coefficient as follows: 
