...
* The EKF method with extra iteration in the correction step perform close to a Generalized Additive model in terms of out-sample Brier score
...
```{r simulations_all_variying, warning=FALSE, cache=FALSE, results='asis'}
set.seed(1243)
...
```{r simulations_one_variying, warning=FALSE, cache=FALSE, results='asis'}
# Use simulation function
set.seed(8080)
simulate_n_print_res(
...
```{r simulations_sim_finer_binning, warning=FALSE, cache=FALSE, results='asis'}
# Use simulation function
set.seed(747)
simulate_n_print_res(
...
## Conclussion on run
The UKF and extra iteration seems to perform well in both settings.
...
# get_expect_equal(tmp$res[tmp$res$id == 25, ])
