...
$\matLarrow{S}_t = \matLarrow{P}_t\mat{F}^\top\matLarrow{P}_{t + 1}^{-1}\mat{R}\mat{Q}\mat{R}^\top\Lparen{\mat{F}^\top}^{-1}$, %
$\mat{R}^{+}\matLarrow{P}_t^{-1}\vecLarrow{m}_t$, and $\mat{R}^{+}\matLarrow{P}_t^{-1}\mat{R}$ for equation~\eqref{eqn:bwTransProp} and \eqref{eqn:bw_approx}. This is done with the virtual methods 
\texttt{bw\_mean(signed int t, const arma::vec \&a)}, %
\texttt{bw\_covar(signed int t)}, %
\texttt{uncond\_mean(signed int t)}, and \texttt{uncond\_covar(signed int t)}. These will computed once using equation~\eqref{eqn:bwTransProp}. 
...
\begin{enumerate}
	...
	\item Given one or two additional pairs of means and covariances $(\mat{S}_1, \vec{a}_1)$, $(\mat{S}_2, \vec{a}_2)$ then compute %
	%
	\begin{align*}
		\mat{\Sigma}^{-1} &= \mat{S}_1^{-1} + \mat{S}_2^{-1} + \mat{K} \\
		\vec{\mu} &= \mat{\Sigma}\Lparen{
		    \mat{S}_1^{-1}\vec{a}_1 + 
		    \mat{S}_2^{-1}\vec{a}_2 + \vec{k}}
	\end{align*}
\end{enumerate}

So a common task in equation~\eqref{eqn:TaylorProposalForward},~\eqref{eqn:bw_approx}~and~\eqref{eqn:approx_smooth} is to compute $\mat{\Sigma}^{-1}$ and $\vec{k}$ for one two $\mat{S}_i$ matrices which is done by a single method.
