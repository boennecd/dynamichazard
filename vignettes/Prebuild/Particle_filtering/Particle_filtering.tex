...
The following sections will closely follow the example shown in \cite{fearnhead10} and the appendix of the paper. The following section uses $\mat{Q}$ throughout in the formulas where it should have been $\mat{R}\mat{Q}\mat{R}^\top$.
...
Further, we get the following backward version of equation~\eqref{eqn:TaylorProposalForward}:
%
\begin{equation}\begin{split}
...
%
	\vecLarrow{\mu}_t 
	 &=  \matLarrow{\Sigma}_t \Lparen{
		...
		\mat{X}_t^\top
		\Lparen{
			-\mat{G}_t\Lparen{\mat{F}^{-1}\vecb{\alpha}_{t + 1}}
...
%
	\vecLRarrow{\mu}_t 
	 &=  \matLRarrow{\Sigma}_t \Lparen{
		...
		\mat{X}_t^\top
		\Lparen{
			-\mat{G}_t\Lparen{\vecb{\alpha}_{t}}
