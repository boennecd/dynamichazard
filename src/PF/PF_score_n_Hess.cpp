  ...
  const bool only_score): weight(exp(p.log_weight))
{
  ...
  const arma::span obs_span(0L, dfixd - 1L), sta_span(dfixd, score_dim - 1L);
  {
    ...
    if(!only_score)
      /* TODO: replace with dsyr */
      hess_terms(obs_span, obs_span) += score(obs_span) * score(obs_span).t();
  }
                   ...
                   Qspan(dfixd + dsatesq, dfixd + 2L * dsatesq - 1L);
  {
    ...
    for(auto parent = old_cl.cbegin(); parent !=  old_cl.cend();
      ...
      /* handle dL/dF */
      const arma::vec innovation = p.get_state() - dat.F * parent->get_state(),
        innovation_Qinv =
          solve_w_precomputed_chol(dat.Q_chol, innovation);
      score_terms_Fd = parent->get_state() * innovation_Qinv.t();

      /* handle dL/dQ */
      score_terms_Qd =  (innovation / 2) * innovation_Qinv.t();
      score_terms_Qd.diag() -= .5;
      score_terms_Qd = solve_w_precomputed_chol(dat.Q_chol, score_terms_Qd);
  ...
  if(!only_score){
    ...
    /* TODO: replace w/ dger? */
    {
      ...
      hess_terms.rows(obs_span) += obs_score_term * tmp.t();
    }

   /* subtract outer product of score */
   /* TODO: could use dsyr */
   hess_terms -= score * score.t();
